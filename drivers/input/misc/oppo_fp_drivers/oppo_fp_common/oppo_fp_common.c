...
#include <linux/proc_fs.h>
	...
	CONFIG_OPPO_FINGERPRINT_PLATFORM == 6779
#include <sec_boot_lib.h>
#elif CONFIG_OPPO_FINGERPRINT_PLATFORM == 855 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 660
// shygosh: add for platform 660
...
#define ENGINEER_MENU_FPC1140                                                  \
	"-1,-1" /* content in " " represents SNR,inclination test item in order in engineer menu, and -1/1 means off/on */
...
#endif
#if CONFIG_OPPO_FINGERPRINT_PROJECT == 18531 ||                                \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18151 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18161
...
#elif CONFIG_OPPO_FINGERPRINT_PROJECT == 17081
fp_module_config_t fp_module_config_list[] = {
...
static int fp_gpio_parse_dts(struct fp_data *fp_data)
{
	...
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18151 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18161
	ret = fp_request_named_gpio(fp_data, "oppo,fp-id0", &fp_data->gpio_id0);
	if (ret) {
		ret = FP_ERROR_GPIO;
		goto exit;
	}
#elif CONFIG_OPPO_FINGERPRINT_PROJECT == 18097 ||                              \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18099 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18397 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18041 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18181 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18115 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18501 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18503
...
#elif CONFIG_OPPO_FINGERPRINT_PROJECT == 17081 ||                              \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18321 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18326 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18323
				...
				size_t count, loff_t *pos)
{
	...
	local_count =
		(FP_ID_MAX_LENGTH - 1) < count ? (FP_ID_MAX_LENGTH - 1) : count;
...
static int fp_get_matched_chip_module(struct device *dev, int fp_id1,
				      int fp_id2, int fp_id3)
{
	int i;
	for (i = 0;
	     i < sizeof(fp_module_config_list) / sizeof(fp_module_config_t);
	     ++i) {
		if ((fp_module_config_list[i].gpio_id_config_list[0] ==
		     fp_id1) &&
		    (fp_module_config_list[i].gpio_id_config_list[1] ==
		     fp_id2) &&
		    (fp_module_config_list[i].gpio_id_config_list[2] ==
		     fp_id3)) {
			switch (fp_module_config_list[i].fp_vendor_chip) {
			case FP_FPC_1022:
				strcpy(fp_manu, CHIP_FPC);
				strcat(fp_manu, "_1022");
				strcpy(g_engineermode_menu_config,
				       fp_module_config_list[i]
					       .engineermode_menu_config);
				return FP_FPC_1022;
			case FP_FPC_1023:
				strcpy(fp_manu, CHIP_FPC);
...
static int fp_register_proc_fs(struct fp_data *fp_data)
{
	...
	fp_id_retry = 0;
#if CONFIG_OPPO_FINGERPRINT_PROJECT == 18181 ||                                \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18385 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18097 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18099 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18397 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18041 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18115 ||                            \
...
#elif CONFIG_OPPO_FINGERPRINT_PROJECT == 18531 ||                              \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18151 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18161
	...
	dev_err(fp_data->dev,
		"fp_register_proc_fs check: fp_id0= %d, fp_id1= %d, fp_id2= %d, fp_id3 = %d, fp_id_retry= %d\n",
		fp_data->fp_id0, fp_data->fp_id1, fp_data->fp_id2,
		fp_data->fp_id3, fp_id_retry);
#if CONFIG_OPPO_FINGERPRINT_PROJECT == 17081 ||                                \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18321 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18326 ||                            \
	CONFIG_OPPO_FINGERPRINT_PROJECT == 18323
...
#else
	fp_data->fpsensor_type =
		fp_get_matched_chip_module(fp_data->dev, fp_data->fp_id0,
					   fp_data->fp_id1, fp_data->fp_id2);
#endif
