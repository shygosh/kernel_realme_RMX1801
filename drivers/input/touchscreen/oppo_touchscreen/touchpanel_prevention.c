			     ...
			     struct point_info cur_p)
{
	...
	list_for_each (pos, &grip_info->dead_zone_list) {
		...
		if ((grip_area->support_dir >> grip_info->touch_dir) & 0x01) {
			if ((cur_p.x <=
			     grip_area->start_x + grip_area->x_width) &&
			    (cur_p.x >= grip_area->start_x) &&
			    (cur_p.y <=
			     grip_area->start_y + grip_area->y_width) &&
		     ...
		     struct point_info *points)
{
	...
	for (i = 0; i < TOUCH_MAX_NUM; i++) {
		if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01) {
			if (grip_info->dead_out_status
				    [i]) //if already outside the range, skip handle
				continue;

			is_exit = dead_grip_judged(grip_info, points[i]);
			grip_info->dead_out_status[i] =
				is_exit; //set outside flag
			if (!is_exit) {
			      ...
			      struct point_info *cur_p)
{
	if (!grip_info->no_handle_dir) { //default right side
		if ((cur_p->x > grip_info->max_x / 2) &&
		    (cur_p->y > grip_info->no_handle_y1) &&
...
static bool large_area_judged(struct kernel_grip_info *grip_info,
			      uint16_t *grip_side, struct point_info *points,
			      int index)
{
	int thd = 0;
	bool is_exit = true;
	...
	list_for_each (pos, &grip_info->large_zone_list) {
		...
		if ((grip_area->support_dir >> grip_info->touch_dir) & 0x01) {
			...
			if ((grip_info->first_point[index].x <=
			     grip_area->start_x + grip_area->x_width) &&
			    (grip_info->first_point[index].x >=
			     grip_area->start_x) &&
			    (grip_info->first_point[index].y <=
			     grip_area->start_y + grip_area->y_width) &&
			    (grip_info->first_point[index].y >=
			     grip_area->start_y)) {
				if (((grip_area->grip_side >>
...
static enum large_judge_status
large_shape_judged(struct kernel_grip_info *grip_info, uint16_t side,
	...
	if (((side >> TYPE_SHORT_CORNER_SIDE) & 0x01) ||
		...
		if (frame <= grip_info->large_corner_frame_limit) {
			TPD_INFO("rx:%d, tx:%d.(%d %d)\n", cur_p.rx_press,
				 cur_p.tx_press, cur_p.x, cur_p.y);
			if ((side >> TYPE_SHORT_CORNER_SIDE) & 0x01) {
				...
				if (area_flag &&
				    (grip_info->first_point[index].tx_press >=
				     thd) &&
				    (cur_p.tx_press >= thd) &&
				    (abs(cur_p.y -
					 grip_info->first_point[index].y) >
			...
			if ((side >> TYPE_LONG_CORNER_SIDE) & 0x01) {
				...
				if (area_flag &&
				    (grip_info->first_point[index].rx_press >=
				     thd) &&
				    (cur_p.rx_press >= thd) &&
				    (abs(cur_p.x -
					 grip_info->first_point[index].x) >
		...
		if (frame <= grip_info->large_frame_limit) {
			...
			if ((side >> TYPE_LONG_SIDE) & 0x01) {
				...
				if (cur_p.rx_press * 100 / cur_p.tx_press >=
				    thd) {
					TPD_INFO(
						"large reject for rx:%d, tx:%d.\n",
			...
			if ((side >> TYPE_SHORT_SIDE) & 0x01) {
				...
				if (cur_p.tx_press * 100 / cur_p.rx_press >=
				    thd) {
					TPD_INFO(
						"large reject for tx:%d, rx:%d.\n",
				  ...
				  struct point_info *points, int index)
{
	...
	list_for_each (pos, &grip_info->condition_zone_list) {
		...
		if ((condition_area->support_dir >> grip_info->touch_dir) &
		    0x01) {
			if ((cur_p.x <= condition_area->start_x +
						condition_area->x_width) &&
			    (cur_p.x >= condition_area->start_x) &&
			    (cur_p.y <= condition_area->start_y +
						condition_area->y_width) &&
			    (cur_p.y >= condition_area->start_y)) {
				if ((condition_area->grip_side >>
				     TYPE_SHORT_SIDE) &
				    0x01) {
					if (abs(grip_info->first_point[index].x -
						cur_p.x) <
					    condition_area->exit_thd)
						return false;
				} else if ((condition_area->grip_side >>
					    TYPE_LONG_SIDE) &
					   0x01) {
					if (abs(grip_info->first_point[index].y -
						cur_p.y) <
					    condition_area->exit_thd)
						return false;
				}
			...
			if ((grip_info->first_point[index].x <=
			     condition_area->start_x +
				     condition_area->x_width) &&
			    (grip_info->first_point[index].x >=
			     condition_area->start_x) &&
			    (grip_info->first_point[index].y <=
			     condition_area->start_y +
				     condition_area->y_width) &&
			    (grip_info->first_point[index].y >=
			     condition_area->start_y)) {
				  ...
				  int obj_attention, struct point_info *points)
{
	...
	for (i = 0; i < TOUCH_MAX_NUM; i++) {
		if (((obj_attention & TOUCH_BIT_CHECK) >> i) &
		    0x01) { //finger down
			grip_info->frame_cnt[i]++; //count down frames

			if (grip_info->large_out_status
				    [i]) { //if already exit large area judge
				if (grip_info->condition_out_status[i]) {
					if ((grip_info->makeup_cnt[i] > 0) &&
						...
						assign_filtered_data(
							grip_info, i,
							&points[i]);
						add_filter_data_tail(grip_info,
								     i,
								     tmp_point);
						grip_info->makeup_cnt[i]++;
						TPD_INFO(
			...
			if (!(((grip_info->obj_prev_bit & TOUCH_BIT_CHECK) >>
			       i) &
				...
				grip_info->first_point[i].tx_press =
					points[i].tx_press;
				grip_info->first_point[i].rx_press =
					points[i].rx_press;
			}

			grip_info->point_unmoved[i] =
				((points[i].x ==
				  grip_info->coord_buf[i * fiter_cnt].x) &&
				 (points[i].y ==
				  grip_info->coord_buf[i * fiter_cnt].y));

			is_exit = large_area_judged(grip_info, &large_side,
						    points, i);
			if (is_exit) {
				grip_info->large_out_status[i] =
					true; //set large outside flag
				grip_info->condition_out_status[i] =
					true; //set condition outside flag

				if (!grip_info->point_unmoved
					     [i]) { //means once in large judge area
					tmp_point = points[i];
					assign_filtered_data(grip_info, i,
							     &points[i]);
					add_filter_data_tail(grip_info, i,
							     tmp_point);
					grip_info->makeup_cnt[i]++;
					TPD_INFO("make up b:%d times.\n",
						 grip_info->makeup_cnt[i]);
				}
			} else if (TYPE_REJECT_DONE ==
				   grip_info->large_reject[i]) {
				obj_final =
					obj_final &
					(~(1
				...
				grip_info->condition_out_status[i] =
					condition_area_judged(grip_info, points,
							      i);

				judge_state = large_shape_judged(
					grip_info, large_side, points, i);
				if (JUDGE_LARGE_OK == judge_state) {
					obj_final =
						obj_final &
						(~(1
						   << i)); //reject for big area
					grip_info->large_reject[i] =
						TYPE_REJECT_DONE;
				} else if (JUDGE_LARGE_TIMEOUT == judge_state) {
					grip_info->large_out_status[i] =
						true; //set outside flag
					...
					if (grip_info->condition_out_status[i]) {
						if (!grip_info->point_unmoved[i]) {
							...
							assign_filtered_data(
								grip_info, i,
								&points[i]); //makeup points
							add_filter_data_tail(
								grip_info, i,
								tmp_point);
							grip_info
								->makeup_cnt[i]++;
			...
			if (!grip_info->large_out_status[i] &&
				...
				dead_out =
					dead_grip_judged(grip_info, points[i]);
				if (dead_out) {
					points[i].status = 1;
					...
					if (hrtimer_active(
						...
						if (!ret) {
							TPD_INFO(
								"large get id failed, empty.\n");
						}
						TPD_INFO(
							"large get id(%d) and cancel timer.\n",
							up_id);
					}

					kfifo_put(
						&grip_info->up_fifo,
						i); //put the up id into up fifo
					TPD_INFO(
						"large put id(%d) into fifo.\n",
						i);
					hrtimer_start(
						&grip_info->grip_up_timer[i],
						ktime_set(
							0,
							grip_info->condition_updelay_ms *
								1000000),
						HRTIMER_MODE_REL);
				} else {
					TPD_INFO(
						"reject id:%d for dead zone.\n",
						i);
				}
			} else if (!grip_info->condition_out_status[i] &&
				   grip_info->point_unmoved[i]) {
				if (grip_info->frame_cnt[i] <
					...
					dead_out = dead_grip_judged(grip_info,
								    points[i]);
					if (dead_out) {
						points[i].status = 1;
						grip_info->grip_hold_status[i] =
							1;
						obj_final =
							obj_final | (1 << i);
					...
					TPD_INFO(
						"conditon reject for down frame:%d(%d).\n",
	...
	for (i = 0; i < TOUCH_MAX_NUM; i++) {
		if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01) {
			if (VERTICAL_SCREEN == grip_info->touch_dir) {
				out_status = eliminated_area_judged(grip_info,
								    points[i]);
				if (!out_status) {
					left_edge_cnt++;
					left_edge_bit =
					...
					grip_info->eli_out_status[i] =
						true; //set cross status of i
				}
			} else {
				if (points[i].y < grip_info->max_y / 2) {
					out_status = eliminated_area_judged(
						grip_info, points[i]);
					if (!out_status) {
						left_edge_cnt++;
			     ...
			     int obj_attention, struct point_info *points)
{
	...
	if (!(grip_info->grip_disable_level & (1 << GRIP_DISABLE_LARGE)))
		obj_attention = large_condition_handle(grip_info, obj_attention,
						       points);
	if (!(grip_info->grip_disable_level & (1 << GRIP_DISABLE_ELI)))
		obj_attention = touch_elimination_handle(grip_info,
							 obj_attention, points);
	...
	for (i = 0; i < TOUCH_MAX_NUM; i++) {
		if (grip_info->grip_hold_status[i]) { //handle hold id
			if (((grip_info->obj_prev_bit & TOUCH_BIT_CHECK) >> i) &
				...
				input_mt_report_slot_state(g_tp->input_dev,
							   MT_TOOL_FINGER, 0);
				grip_status_reset(grip_info,
						  i); //reset status of this ID

				if (grip_info->record_total_cnt)
					grip_info->record_total_cnt--; //update touch down count
				grip_info->grip_hold_status[i] = 0;
				TPD_INFO(
					"id:%d report touch up firstly, left total(%d).\n",
					i, grip_info->record_total_cnt);
			} else if (!grip_info->eli_reject_status
					    [i]) { //aviod hold points be cleared by next down frame
				obj_attention =
					obj_attention |
					(1
	...
	grip_info->record_total_cnt =
		get_bit_count(obj_attention & TOUCH_BIT_CHECK);
	return obj_attention;
}
			   ...
			   struct kernel_grip_info *grip_info)
{
	...
	list_for_each (pos, &grip_info->dead_zone_list) {
		...
		seq_printf(
			...
			grip_area->x_width, grip_area->y_width,
			grip_area->exit_thd, grip_area->support_dir,
			grip_area->grip_side);
	}
	seq_printf(s, "\n");
	...
	seq_printf(
		s,
		"large_frame_limit:%4d, large_ver_thd:%4d, large_hor_thd:%4d\n",
...
large_ver_corner_width:%4d, large_hor_corner_width:%4d, large_corner_distance:%4d\n",
		...
		grip_info->large_ver_corner_thd,
		...
		if (in[i] < '0' || in[i] > '9') {
			if ((' ' == in[i]) || (0 == in[i]) || ('\n' == in[i]) ||
				...
				if (value_cnt <= array_max) {
					array[value_cnt++] = str_to_int(
						in, start_pos, i - 1);
					start_pos = i + 1;
				} else {
					TPD_INFO(
						"%s: too many parameter(%s).\n",
						__func__, in);
					return -1;
				}
			} else {
				...
				bool is_add, struct kernel_grip_info *grip_info)
{
	...
	ret = str_parse(in, name, GRIP_TAG_SIZE, split_value,
			MAX_AREA_PARAMETER);
	if (ret < 0) {
	...
	if (is_add) {
		...
		if (grip_info->grip_handle_in_fw && op &&
		    op->set_fw_grip_area && g_tp && !g_tp->loading_fw) {
		...
		list_for_each (pos, handle_list) {
			...
			if (strstr(in, grip_zone->name)) {
				...
				if (grip_info->grip_handle_in_fw && op &&
				    op->set_fw_grip_area && g_tp &&
				    !g_tp->loading_fw) {
					ret = op->set_fw_grip_area(grip_zone,
								   true);
					if (ret < 0) {
						TPD_INFO(
							"%s: set grip area in fw failed !\n",
							__func__);
						return ret;
					}
				}
			     ...
			     int len)
{
	...
	for (i = 1; i < len; i++) {
		if ((' ' == input[i]) ||
		    (len - 1 == i)) { //find a space or to the end
			if (' ' == input[i]) {
				end_pos = i - 1;
			} else {
				end_pos = i;
		...
		if (OBJECT_PARAMETER == object) {
			if (OPERATE_MODIFY == cmd) {
				if ((value = get_key_value(
					     split_str[i],
					     "condition_frame_limit")) >= 0) {
					grip_info->condition_frame_limit =
						value;
					TPD_INFO(
						"change condition_frame_limit to %d.\n",
						value);
					if (grip_info->grip_handle_in_fw &&
					    op &&
					    op->set_condition_frame_limit &&
					    g_tp && !g_tp->loading_fw) {
						ret = op->set_condition_frame_limit(
							value);
						if (ret < 0) {
							TPD_INFO(
								"%s: set condition frame limit in fw failed !\n",
								__func__);
						}
					}
				} else if ((value = get_key_value(
						    split_str[i],
						    "condition_updelay_ms")) >=
					   0) {
					grip_info->condition_updelay_ms = value;
					TPD_INFO(
						"change condition_updelay_ms to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "large_frame_limit")) >=
					...
					TPD_INFO(
						"change large_frame_limit to %d.\n",
						value);
					if (grip_info->grip_handle_in_fw &&
					    op && op->set_large_frame_limit &&
					    g_tp && !g_tp->loading_fw) {
						ret = op->set_large_frame_limit(
							value);
						if (ret < 0) {
							TPD_INFO(
								"%s: set condition frame limit in fw failed !\n",
					...
					TPD_INFO(
						"change large_ver_thd to %d.\n",
						value);
					if (grip_info->grip_handle_in_fw &&
					    op && op->set_large_ver_thd &&
					    g_tp && !g_tp->loading_fw) {
						ret = op->set_large_ver_thd(
							value);
						if (ret < 0) {
							TPD_INFO(
								"%s: set large ver thd in fw failed !\n",
								__func__);
						}
					}
				} else if ((value = get_key_value(
					...
					TPD_INFO(
						"change large_hor_thd to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "large_hor_corner_thd")) >=
					   0) {
					grip_info->large_hor_corner_thd = value;
					TPD_INFO(
						"change large_hor_corner_thd to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "large_ver_corner_thd")) >=
					   0) {
					grip_info->large_ver_corner_thd = value;
					TPD_INFO(
						"change large_ver_corner_thd to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "large_corner_frame_limit")) >=
					   0) {
					grip_info->large_corner_frame_limit =
						value;
					TPD_INFO(
						"change large_corner_frame_limit to %d.\n",
						value);
					if (grip_info->grip_handle_in_fw &&
					    op &&
					    op->set_large_corner_frame_limit &&
					    g_tp && !g_tp->loading_fw) {
						ret = op->set_large_corner_frame_limit(
							value);
						if (ret < 0) {
							TPD_INFO(
								"%s: set large condition frame limit in fw failed !\n",
								__func__);
						}
					}
				} else if ((value = get_key_value(
					...
					grip_info->large_ver_corner_width =
						value;
					TPD_INFO(
						"change large_ver_corner_width to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "large_hor_corner_width")) >=
					...
					grip_info->large_hor_corner_width =
						value;
					TPD_INFO(
						"change large_hor_corner_width to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "large_corner_distance")) >=
					   0) {
					grip_info->large_corner_distance =
						value;
					TPD_INFO(
						"change large_corner_distance to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "grip_disable_level")) >=
					...
					grip_info->grip_disable_level |=
						1 << value;
					TPD_INFO(
						"change grip_disable_level to %d.\n",
						value);
				} else if ((value = get_key_value(
						    split_str[i],
						    "grip_enable_level")) >=
				...
				TPD_INFO(
					"not support %d opeartion for parameter modify.\n",
					cmd);
			}
		} else if ((OBJECT_CONDITION_AREA == object) ||
			   (OBJECT_LARGE_AREA == object) ||
			   (OBJECT_ELI_AREA == object) ||
			   (OBJECT_DEAD_AREA == object)) {
			if (OPERATE_ADD == cmd) {
				TPD_DETAIL("add %d by %s.\n", object,
					   split_str[i]);
				grip_area_add_modify(
					handle_list, split_str[i], true,
					grip_info); //add new area to list
			} else if (OPERATE_DELTE == cmd) {
				TPD_DETAIL("del %d by %s.\n", object,
					   split_str[i]);
				grip_area_del(handle_list, split_str[i],
					      grip_info); //del area from list
			} else if (OPERATE_MODIFY == cmd) {
				TPD_DETAIL("modify %d by %s.\n", object,
					   split_str[i]);
				grip_area_add_modify(
					handle_list, split_str[i], false,
					grip_info); //modify paramter of the area
			} else {
				TPD_INFO(
					"not support %d opeartion for area modify.\n",
					cmd);
			}
			...
			if (OPERATE_MODIFY == cmd) {
				...
				TPD_INFO(
					"not support %d opeartion for skip handle modify.\n",
					cmd);
			}
		}
			   ...
			   struct kernel_grip_info *grip_info)
{
	...
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO(
			"%s: Couldn't create oplus_tp_direction proc entry, %d\n",
			__func__, __LINE__);
	}
}
...
struct kernel_grip_info *kernel_grip_init(struct device *dev)
{
	...
	int dead_width[2] = { 0 }, makeup_para[10] = { 0 },
	    large_para[3] = { 0 }, cond_para[2] = { 0 },
	    no_handle_para[3] = { 0 };
	int temp_array[2] = { 0 }, cond_width[4] = { 0 },
	    large_width[2] = { 0 }, large_corner_width[3] = { 0 },
	    eli_width[6] = { 0 };
	struct grip_zone_area *grip_zone = NULL;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "touchpanel,panel-coords", temp_array, 2);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,grip_disable_level", temp_array, 1);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,no_handle_para", no_handle_para, 3);
	if (ret) {
		no_handle_para[0] = 0;
		no_handle_para[1] = 0;
		no_handle_para[2] = 0;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,dead_area_width", dead_width, 2);
	if (ret) {
		dead_width[0] = 10;
		dead_width[1] = 10;
		TPD_INFO("panel coords using default.\n");
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,makeup_cnt_weight", makeup_para, 5);
	if (ret) {
		makeup_para[0] = 4;
		makeup_para[1] = 1;
		makeup_para[2] = 2;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,large_judge_para", large_para, 3);
	if (ret) {
		large_para[0] = 3;
		large_para[1] = 300;
		large_para[2] = 300;
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "prevention,large_corner_judge_para",
					 large_corner_para, 6);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,condition_judge_para", cond_para, 2);
	if (ret) {
		cond_para[0] = 40;
		cond_para[1] = 50;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,condition_area_width", cond_width, 4);
	if (ret) {
		cond_width[0] = 30;
		cond_width[1] = 30;
		cond_width[2] = 100;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,large_area_width", large_width, 2);
	if (ret) {
		large_width[0] = 100;
		large_width[1] = 100;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "prevention,eli_area_width", eli_width, 6);
	if (ret) {
		eli_width[0] = 120;
		eli_width[1] = 500;
		eli_width[2] = 250;
	...
	grip_info->grip_handle_in_fw = of_property_read_bool(
		dev->of_node, "prevention,grip_handle_in_fw");
	if (grip_info->grip_handle_in_fw) {
	...
	if (grip_zone) {
		...
		grip_zone->support_dir = (1 << VERTICAL_SCREEN) |
					 (1 << LANDSCAPE_SCREEN_90) |
					 (1 << LANDSCAPE_SCREEN_270);
		list_add_tail(&grip_zone->area_list,
			      &grip_info->dead_zone_list);
	} else {
	...
	grip_info->coord_buf =
		kzalloc(TOUCH_MAX_NUM * grip_info->coord_filter_cnt *
				sizeof(struct coord_buffer),
			GFP_KERNEL);
	if (grip_info->coord_buf) {
		for (i = 0; i < TOUCH_MAX_NUM; i++) {
			for (j = 0; j < grip_info->coord_filter_cnt; j++) {
				grip_info
					->coord_buf
						[i * grip_info->coord_filter_cnt +
						 j]
					.weight = makeup_para[j + 1];
			}
		}
	} else {
	...
	if (grip_zone) {
		...
		snprintf(grip_zone->name, GRIP_TAG_SIZE - 1,
			 "ver_left_condtion");
		grip_zone->grip_side = 1 << TYPE_LONG_SIDE;
		grip_zone->support_dir = (1 << VERTICAL_SCREEN) |
					 (1 << LANDSCAPE_SCREEN_90) |
	...
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for ver_right_condtion failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_left_condtion failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_right_condtion failed.\n");
	}
	...
	if (grip_zone) {
		...
		grip_zone->support_dir = (1 << LANDSCAPE_SCREEN_90) |
					 (1 << LANDSCAPE_SCREEN_270);
		list_add_tail(&grip_zone->area_list,
			      &grip_info->large_zone_list);
	} else {
		TPD_INFO(
			"kzalloc grip_zone_area for hor90_left_large_corner failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor90_right_large_corner failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor270_left_large_corner failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = grip_info->max_x - large_corner_width[1];
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor270_right_large_corner failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for ver_left_bottom_large failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for ver_right_bottom_large failed.\n");
	}
	...
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_right_large failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
	...
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for ver_right_large failed.\n");
	}
	...
	for (i = 0; i < TOUCH_MAX_NUM; i++) {
		hrtimer_init(&grip_info->grip_up_timer[i], CLOCK_MONOTONIC,
			     HRTIMER_MODE_REL);
		grip_info->grip_up_timer[i].function = touch_up_timer_func;
		INIT_WORK(&grip_info->grip_up_work[i], touch_report_work);
	}
	grip_info->grip_up_handle_wq =
		create_singlethread_workqueue("touch_up_wq");
	...
	if (grip_zone) {
		...
		grip_zone->support_dir = 1 << VERTICAL_SCREEN;
		list_add_tail(&grip_zone->area_list,
			      &grip_info->elimination_zone_list);
	} else {
	...
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_90_left_0_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_90_left_1_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_90_right_0_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = 0;
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_90_right_1_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_270_left_0_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_270_left_1_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = grip_info->max_x - eli_width[2];
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_270_right_0_eli failed.\n");
	}
	grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
	if (grip_zone) {
		grip_zone->start_x = grip_info->max_x - eli_width[5];
		...
		TPD_INFO(
			"kzalloc grip_zone_area for hor_270_right_1_eli failed.\n");
	}
