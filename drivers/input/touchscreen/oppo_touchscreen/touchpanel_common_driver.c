...
notify_prevention_handle(struct kernel_grip_info *grip_info, int obj_attention,
...
void operate_mode_switch(struct touchpanel_data *ts)
{
	...
	if (ts->is_suspended) {
		...
		if (ts->kernel_grip_support && ts->ts_ops->enable_kernel_grip) {
			ts->ts_ops->enable_kernel_grip(ts->chip_data,
						       grip_info);
		}
...
static void tp_touch_handle(struct touchpanel_data *ts)
{
	...
	if (ts->kernel_grip_support) {
		obj_attention = notify_prevention_handle(ts->grip_info,
		...
		for (i = 0; i < ts->max_num; i++) {
			...
			if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01 &&
				...
				if (ts->kernel_grip_support && ts->grip_info &&
				    ...
				    !(ts->grip_info->grip_disable_level &
				      (1 << GRIP_DISABLE_UP2CANCEL))) {
					input_report_abs(
						ts->input_dev, ABS_MT_PRESSURE,
						UP2CANCEL_PRESSURE_VALUE);
				}
				input_mt_report_slot_state(ts->input_dev,
							   MT_TOOL_FINGER, 0);
			}
...
static void tp_btnkey_handle(struct touchpanel_data *ts)
{
	...
	if (CHK_BIT(ts->vk_bitmap, BIT_MENU))
		input_report_key_oppo(ts->kpd_input_dev, KEY_MENU,
				      CHK_BIT(touch_state, BIT_MENU));
	if (CHK_BIT(ts->vk_bitmap, BIT_HOME))
		input_report_key_oppo(ts->kpd_input_dev, KEY_HOMEPAGE,
				      CHK_BIT(touch_state, BIT_HOME));
	if (CHK_BIT(ts->vk_bitmap, BIT_BACK))
		input_report_key_oppo(ts->kpd_input_dev, KEY_BACK,
				       ...
				       size_t count, loff_t *ppos)
{
	...
	if (ts->ts_ops->register_info_read) {
		...
		for (i = 0; i < ts->reg_info.reg_length; i++) {
			num_read_chars +=
				snprintf(&(page[num_read_chars]), PAGESIZE - 1,
					 "reg_addr(0x%x) = 0x%x\n",
					 ts->reg_info.reg_addr,
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	if (ts->edge_limit_support || ts->fw_edge_limit_support) {
		...
		prEntry_tmp =
			proc_create_data("oplus_tp_direction", 0666, prEntry_tp,
		...
		if (ts->edge_limit_support) {
			prEntry_tmp =
				proc_create_data("oppo_tp_limit_area", 0664,
						 prEntry_tp,
	...
	if (ts->spurious_fp_support) {
		...
		prEntry_tmp =
			proc_create_data("finger_protect_trigger", 0666,
					 prEntry_tp,
	...
	if (ts->noise_modetest_support) {
		prEntry_tmp = proc_create_data("oppo_tp_noise_modetest", 0664,
					       prEntry_tp,
	...
	if (ts->charger_pump_support) {
		prEntry_tmp =
			proc_create_data("charge_detect", 0666, prEntry_tp,
	...
	if (ts->wireless_charger_support) {
		prEntry_tmp =
			proc_create_data("wireless_charge_detect", 0666,
					 prEntry_tp,
...
static int init_debug_info_proc(struct touchpanel_data *ts)
{
	...
	if (ts->health_monitor_support) {
		...
		prEntry_tmp =
			proc_create_data("health_monitor", 0666,
					 prEntry_debug_info,
	...
	if (ts->ear_sense_support) {
		...
		prEntry_tmp =
			proc_create_data("es_touch_count", 0666,
					 prEntry_earsense,
	...
	if (ts->fingerprint_underscreen_support) {
		prEntry_tmp =
			proc_create_data("fp_enable", 0666, ts->prEntry_tp,
	...
	if (ts->report_point_first_support) {
		prEntry_tmp =
			proc_create_data("report_point_first", 0666,
					 ts->prEntry_tp,
...
static int init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
	...
	ts->kernel_grip_support =
		of_property_read_bool(np, "kernel_grip_support");
