...
static uint8_t nvt_wdt_fw_recovery(struct chip_data_nt36672 *chip_info,
				   uint8_t *point_data)
{
		...
		for (list = 0; list < (sizeof(trim_id_table) /
			...
			for (i = 0; i < NVT_ID_BYTE_MAX; i++) {
				if (trim_id_table[list].mask[i]) {
					if (buf[i + 1] !=
					    trim_id_table[list]
						    .id[i]) //set parameter from chip id
						break;
				}
			}
...
*******************************************************/
static int32_t
nvt_check_fw_reset_state_noflash(struct chip_data_nt36672 *chip_info,
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		if (retry_count < 3) {
			...
			TPD_INFO(
				"Set default fw_ver=0, x_num=18, y_num=32, abs_x_max=1080, abs_y_max=1920, max_button_num=0!\n");
			ret = -1;
		}
	} else {
		ret = 0;
...
static int32_t nvt_bin_header_parser(struct chip_data_nt36672 *chip_info,
				     const u8 *fwdata, size_t fwsize)
{
	...
	chip_info->bin_map = (struct nvt_ts_bin_map *)kzalloc(
	...
	for (list = 0; list < chip_info->partition; list++) {
		...
		if (list < chip_info->ilm_dlm_num) {
			...
			if (chip_info->trim_id_table.support_hw_crc)
				...
				if ((chip_info->bin_map[list].BIN_addr +
					...
					chip_info->bin_map[list].crc = CheckSum(
						&fwdata[chip_info->bin_map[list]
								.BIN_addr],
						chip_info->bin_map[list].size);
				else {
					TPD_INFO(
						"access range (0x%08X to 0x%08X) is larger than bin size!\n",
						chip_info->bin_map[list]
							.BIN_addr,
						chip_info->bin_map[list]
								.BIN_addr +
							chip_info->bin_map[list]
		...
		if ((list >= chip_info->ilm_dlm_num) &&
			...
			if ((chip_info->bin_map[list].BIN_addr == 0) &&
				...
				sprintf(chip_info->bin_map[list].name,
					"Header");
			} else {
				sprintf(chip_info->bin_map[list].name,
					"Info-%d",
		...
		if (list >= (chip_info->ilm_dlm_num + info_sec_num)) {
			...
			sprintf(chip_info->bin_map[list].name, "Overlay-%d",
				(list - chip_info->ilm_dlm_num - info_sec_num));
		}
...
static int32_t Download_Init(struct chip_data_nt36672 *chip_info)
{
	...
	if (chip_info->fwbuf == NULL) {
		chip_info->fwbuf =
			(uint8_t *)kzalloc((SPI_TANSFER_LEN + 1), GFP_KERNEL);
		if (chip_info->fwbuf == NULL) {
...
return:
        n.a.
*******************************************************/
static void nvt_read_ram_test(struct chip_data_nt36672 *chip_info,
...
return:
        n.a.
*******************************************************/
static int32_t Write_Partition(struct chip_data_nt36672 *chip_info,
			       const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	char *name;
...
static void nvt_boot_ready(struct chip_data_nt36672 *chip_info, uint8_t ready)
{
	...
	if (!chip_info->trim_id_table.support_hw_crc) {
		//---write BOOT_RDY status cmds---
		nvt_write_addr(chip_info->s_client,
			       chip_info->trim_id_table.mmap->BOOT_RDY_ADDR, 0);

		//---write POR_CD cmds---
		nvt_write_addr(chip_info->s_client,
			       chip_info->trim_id_table.mmap->POR_CD_ADDR,
			       0xA0);
	}
...
static int32_t Check_CheckSum(struct chip_data_nt36672 *chip_info)
{
	...
	/* read checksum */
	chip_info->fwbuf[0] =
		(chip_info->trim_id_table.mmap->R_ILM_CHECKSUM_ADDR) & 0x7F;
	...
	for (list = 0; list < chip_info->partition; list++) {
		...
		if (chip_info->bin_map[list].crc != fw_checksum) {
			TPD_INFO(
				"[%d] BIN_checksum=0x%08X, FW_checksum=0x%08X\n",
				list, chip_info->bin_map[list].crc,
				fw_checksum);
...
return:
        n.a.
*******************************************************/
static int32_t Download_Firmware_HW_CRC(struct chip_data_nt36672 *chip_info,
...
return:
        n.a.
*******************************************************/
static int32_t Download_Firmware(struct chip_data_nt36672 *chip_info,
				 const struct firmware *fw)
{
	uint8_t retry = 0;
	int32_t ret = 0;
...
{
	int ret = -1;
	struct chip_data_nt36672 *chip_info =
		(struct chip_data_nt36672 *)chip_data;
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	if (ret) {
		TPD_INFO(
			"receive all %02X, no gesture interrupts. recover for fw reset\n",
			point_data[1]);
		nvt_reset(chip_info);
		...
		if (chip_info->g_fw == NULL && !is_oem_unlocked()) {
			...
			if (ts->fw_update_app_support) {
				ret = request_firmware_select(
					&chip_info->g_fw, chip_info->fw_name,
					chip_info->dev);
			} else {
				ret = request_firmware(&chip_info->g_fw,
						       chip_info->fw_name,
						       chip_info->dev);
			}
			if (ret != 0) {
				TPD_INFO(
					"%s : request firmware failed! ret = %d\n",
					__func__, ret);
...
static int nvt_enable_black_gesture(struct chip_data_nt36672 *chip_info,
				    bool enable)
{
	int ret = -1;
	uint8_t buf[3];
...
static int nvt_enable_edge_limit(struct chip_data_nt36672 *chip_info, int state)
{
	...
	struct touchpanel_data *ts = spi_get_drvdata(chip_info->s_client);
	TPD_INFO(
		"%s:state = %d, limit_corner = %d, chip_info->is_sleep_writed = %d\n",
	...
	if (state == 1 || VERTICAL_SCREEN == chip_info->touch_direction) {
		...
		if (LANDSCAPE_SCREEN_90 == chip_info->touch_direction) {
			ret = nvt_cmd_store(chip_info,
					    EVENTBUFFER_EDGE_LIMIT_RIGHT_UP);
		} else if (LANDSCAPE_SCREEN_270 == chip_info->touch_direction) {
			ret = nvt_cmd_store(chip_info,
					    EVENTBUFFER_EDGE_LIMIT_LEFT_UP);
		}
	}
...
static int nvt_enable_charge_mode(struct chip_data_nt36672 *chip_info,
				  bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_jitter_mode(struct chip_data_nt36672 *chip_info,
				  bool enable)
{
	int8_t ret = -1;
		...
		if (panel_data->manufacture_info.version) {
			...
			if (ver_len <= 8) {
				strlcat(panel_data->manufacture_info.version,
					dev_version, MAX_DEVICE_VERSION_LENGTH);
			} else {
				strlcpy(&panel_data->manufacture_info.version[8],
					dev_version,
					MAX_DEVICE_VERSION_LENGTH - 8);
			}
		}
	}
...
static void nvt_enable_noise_collect(struct chip_data_nt36672 *chip_info,
				     int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
...
static int8_t nvt_switch_FreqHopEnDis(struct chip_data_nt36672 *chip_info,
...
static void nvt_read_mdata(struct chip_data_nt36672 *chip_info,
			   ...
			   int32_t xdata_len)
{
	...
	if (xdata_len / sizeof(int32_t) < data_len / 2) {
		TPD_INFO(
			"xdata read buffer(%d) less than max data size(%d), return\n",
			xdata_len, data_len);
		return;
	}
...
static int32_t
nvt_polling_hand_shake_status(struct chip_data_nt36672 *chip_info)
{
	uint8_t buf[8] = { 0 };
	int32_t i = 0;
...
static int32_t nvt_read_fw_noise(struct chip_data_nt36672 *chip_info,
	...
	if (fw == NULL) {
		...
		if (chip_info->p_firmware_headfile->firmware_data) {
			request_fw_headfile->size =
				chip_info->p_firmware_headfile->firmware_size;
			request_fw_headfile->data =
				chip_info->p_firmware_headfile->firmware_data;
...
*/
	...
	if (fw != NULL && fw->size != FW_BIN_SIZE) {
		TPD_INFO(
			"bin file size not match (%zu), change to use headfile fw.\n",
			fw->size);
		goto download_fail;
		//release_firmware(fw);
		//fw = NULL;
...
static void nvt_black_screen_test(void *chip_data, char *message)
{
	...
	sprintf(data_buf,
		"/sdcard/TpTestReport/screenOff/tp_testlimit_%02d%02d%02d-%02d%02d%02d-utc.csv",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((raw_data[iArrayIndex] >
					...
					TPD_INFO(
						"LPWG_Rawdata Test failed at rawdata[%d][%d] = %d\n",
	...
	if (nvt_read_fw_noise(chip_info, ph->config_Diff_Test_Frame, raw_data,
			      raw_data_n, buf_len) != 0) {
	...
	if ((ph->config_Lmt_LPWG_Diff_P != 0) &&
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((raw_data[iArrayIndex] >
					...
					TPD_INFO(
						"LPWG Noise RawData_Diff_Max Test failed at rawdata[%d][%d] = %d\n",
						i, j, raw_data[iArrayIndex]);
					if (!err_cnt) {
						sprintf(buf,
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if (fd >= 0) {
					store_to_file(fd, "%d, ",
						      raw_data_n[iArrayIndex]);
				}
				if ((raw_data_n[iArrayIndex] >
				     ph->config_Lmt_LPWG_Diff_P) ||
				    (raw_data_n[iArrayIndex] <
				     ph->config_Lmt_LPWG_Diff_N)) {
					TPD_INFO(
						"LPWG Noise RawData_Diff_Min Test failed at rawdata[%d][%d] = %d\n",
...
static void nvt_enable_doze_noise_collect(struct chip_data_nt36672 *chip_info,
					  int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
...
static int32_t nvt_read_doze_fw_noise(struct chip_data_nt36672 *chip_info,
...
static int32_t nvt_read_doze_baseline(struct chip_data_nt36672 *chip_info,
				      int32_t doze_X_Channel, int32_t *xdata,
				      int32_t xdata_len)
{
	uint8_t buf[256] = { 0 };
	...
	for (x = 0; x < doze_X_Channel; x++) {
		...
		nvt_set_page(chip_info,
			     chip_info->trim_id_table.mmap
					     ->DOZE_GM_S1D_SCAN_RAW_ADDR +
				     rm_num * doze_X_Channel * x);

		//---read data---
		buf[0] = (chip_info->trim_id_table.mmap
				  ->DOZE_GM_S1D_SCAN_RAW_ADDR +
			  rm_num * doze_X_Channel * x) &
			 0xFF;
		CTP_SPI_READ(chip_info->s_client, buf, rm_num * 2 + 1);
		for (y = 0; y < rm_num; y++) {
			xdata[y * 2 + x] = (uint16_t)(buf[y * 2 + 1] +
						      256 * buf[y * 2 + 2]);
		}
	}
...
static int32_t nvt_read_fw_short(struct chip_data_nt36672 *chip_info,
				 int32_t *xdata, int32_t xdata_len)
{
	...
	rawdata_buf = (uint8_t *)kzalloc(chip_info->hw_res->TX_NUM *
...
static int32_t nvt_read_fw_open(struct chip_data_nt36672 *chip_info,
				int32_t *xdata, int32_t xdata_len)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
			  ...
			  struct nvt_testdata *nvt_testdata)
{
	...
	if (nvt_switch_FreqHopEnDis(chip_info, FREQ_HOP_DISABLE)) {
		...
		store_to_file(nvt_testdata->fd,
			      "switch frequency hopping disable failed!\n");
	...
	fw_rawdata_P = (int32_t *)(nvt_testdata->fw->data +
				   ph->array_fw_rawdata_P_offset);
	fw_rawdata_N = (int32_t *)(nvt_testdata->fw->data +
				   ph->array_fw_rawdata_N_offset);
	...
	diff_rawdata_P = (int32_t *)(nvt_testdata->fw->data +
				     ph->array_FW_Diff_P_offset);
	diff_rawdata_N = (int32_t *)(nvt_testdata->fw->data +
				     ph->array_FW_Diff_N_offset);
	cc_data_P =
	...
	doze_diff_rawdata_P = (int32_t *)(nvt_testdata->fw->data +
					  ph->array_Doze_Diff_P_offset);
	doze_diff_rawdata_N = (int32_t *)(nvt_testdata->fw->data +
					  ph->array_Doze_Diff_N_offset);
	...
	for (j = 0; j < nvt_testdata->RX_NUM; j++) {
		for (i = 0; i < nvt_testdata->TX_NUM; i++) {
			...
			if (nvt_testdata->fd >= 0) {
				store_to_file(nvt_testdata->fd, "%d, ",
					      raw_data[iArrayIndex]);
			}
			if ((raw_data[iArrayIndex] >
				...
				TPD_INFO(
					"rawdata Test failed at rawdata[%d][%d] = %d [%d,%d]\n",
					i, j, raw_data[iArrayIndex],
					fw_rawdata_N[iArrayIndex],
					fw_rawdata_P[iArrayIndex]);
				if (!err_cnt) {
	...
	if (nvt_read_doze_fw_noise(chip_info, ph->config_Doze_Noise_Test_Frame,
				   ph->doze_X_Channel, raw_data, raw_data_n,
				   buf_len) != 0) {
		TPD_INFO("read Doze Noise data failed!\n");
		store_to_file(nvt_testdata->fd,
			      "read Doze Noise data failed!\n");
	...
	memset(raw_data, 0, buf_len);
	if (nvt_read_doze_baseline(chip_info, ph->doze_X_Channel, raw_data,
				   buf_len) != 0) {
		TPD_INFO("read Doze FW Rawdata failed!\n");
		store_to_file(nvt_testdata->fd,
			      "read Doze FW Rawdata failed!\n");
