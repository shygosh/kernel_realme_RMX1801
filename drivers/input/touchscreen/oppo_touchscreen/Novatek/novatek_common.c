...
void nvt_flash_proc_init(struct touchpanel_data *ts, const char *name)
{
	...
	if (strstr(name, "SPI")) {
		...
		nvt_proc_entry = proc_create_data(name, 0444, NULL,
						  &nvt_noflash_fops, ts);
...
void nvt_limit_read(struct seq_file *s, struct touchpanel_data *ts)
{
...
#ifdef CONFIG_TOUCHPANEL_NT_DIGITALNOISE_TEST
	digital_diff_rawdata_p =
		(int32_t *)(fw->data + ph->array_FW_Digital_Diff_P_offset);
	digital_diff_rawdata_n =
		(int32_t *)(fw->data + ph->array_FW_Digital_Diff_N_offset);
#endif
	...
	if (ph->config_Lmt_Short_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
					s, "%4d, ",
					short_rawdata_p[i * ts->hw_res.TX_NUM +
							j]);
			}
		}
	}
	...
	if (ph->config_Lmt_Short_Rawdata_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
					s, "%4d, ",
					short_rawdata_n[i * ts->hw_res.TX_NUM +
							j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FW_Diff_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
					s, "%4d, ",
					diff_rawdata_p[i * ts->hw_res.TX_NUM +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FW_Diff_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
					s, "%4d, ",
					diff_rawdata_n[i * ts->hw_res.TX_NUM +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FW_Digital_Diff_P != 0) {
		seq_printf(s, "\n config_Lmt_FW_Digital_Diff_P: %4d",
			   ph->config_Lmt_FW_Digital_Diff_P);
	} else {
		seq_printf(s, "\n Digital_Diff_P:");
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			seq_printf(s, "\n[%2d] ", i);
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(s, "%4d, ",
					   digital_diff_rawdata_p
						   [i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_FW_Digital_Diff_N != 0) {
		seq_printf(s, "\n config_Lmt_FW_Digital_Diff_N: %4d",
			   ph->config_Lmt_FW_Digital_Diff_N);
	} else {
		seq_printf(s, "\n Digital_Diff_N:");
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			seq_printf(s, "\n[%2d] ", i);
	...
	if (ph->config_Lmt_Doze_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->doze_X_Channel; j++) {
				seq_printf(
					s, "%4d, ",
					doze_rawdata_p[i * ph->doze_X_Channel +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_Doze_Rawdata_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->doze_X_Channel; j++) {
				seq_printf(
					s, "%4d, ",
					doze_rawdata_n[i * ph->doze_X_Channel +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_LPWG_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
					s, "%4d, ",
					lpwg_rawdata_p[i * ts->hw_res.TX_NUM +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_LPWG_Rawdata_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
					s, "%4d, ",
					lpwg_rawdata_n[i * ts->hw_res.TX_NUM +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FDM_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->fdm_X_Channel; j++) {
				seq_printf(s, "%4d, ",
					   fdm_rawdata_p[i * ph->fdm_X_Channel +
							 j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FDM_Rawdata_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->fdm_X_Channel; j++) {
				seq_printf(s, "%4d, ",
					   fdm_rawdata_n[i * ph->fdm_X_Channel +
							 j]);
			}
		}
	}
