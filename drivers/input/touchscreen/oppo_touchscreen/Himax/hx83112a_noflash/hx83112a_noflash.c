 ...
 * VENDOR_EDIT
 * File       : hx83112a_nf_drivers_s4322.c
 * Description: Source file for hx83112a_nf S4322 driver
 * Version   : 1.0
 * Date        : 2017-04-17
...
const struct mt_chip_conf hx83112a_nf_hx_spi_ctrdata = {
	...
	defined(CONFIG_TOUCHPANEL_MTK_PLATFORM) // For Checksum Fail On MTK Platform
static ssize_t hx83112a_nf_spi_sync(struct touchpanel_data *ts,
...
static int hx83112a_nf_spi_read(uint8_t *command, uint8_t command_len,
...
#else
	result = spi_write_then_read(hx83112a_nf_pri_ts->s_client,
...
static int hx83112a_nf_bus_write(uint8_t command, uint32_t length,
				 uint8_t *data)
{
...
static uint32_t hx83112a_nf_hw_check_CRC(uint8_t *start_addr,
					 int reload_length);
static fw_check_state
hx83112a_nf_fw_check(void *chip_data, struct resolution_info *resolution_info,
...
void hx83112a_nf_flash_write_burst_length(uint8_t *reg_byte,
...
void hx83112a_nf_register_read(uint8_t *read_addr, int read_length,
			       uint8_t *read_data, bool hx83112a_nf_cfg_flag)
{
	uint8_t tmp_data[4];
	int ret;
	if (hx83112a_nf_cfg_flag == false) {
		...
		if (hx83112a_nf_bus_read(read_addr[0], read_length, read_data) <
		    0) {
			TPD_INFO("%s: i2c access fail!\n", __func__);
			return;
		}
...
void hx83112a_nf_register_write(uint8_t *write_addr, int write_length,
				uint8_t *write_data, bool hx83112a_nf_cfg_flag)
{
	int i = 0;
	int address = 0;
	if (hx83112a_nf_cfg_flag == false) {
		...
		for (i = address; i < address + write_length; i++) {
			...
			hx83112a_nf_flash_write_burst_length(
				write_addr, write_data, write_length);
		}
	} else if (hx83112a_nf_cfg_flag == true) {
		if (hx83112a_nf_bus_write(write_addr[0], write_length,
					  write_data) < 0) {
			TPD_INFO("%s: i2c access fail!\n", __func__);
			return;
		}
...
static int hx83112a_nf_mcu_register_write(uint8_t *write_addr,
					  uint32_t write_length,
					  uint8_t *write_data,
					  uint8_t hx83112a_nf_cfg_flag)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_bus_size) {
			memcpy(tmp_data, write_data + (i * max_bus_size),
			       max_bus_size);
			hx83112a_nf_flash_write_burst_length(tmp_addr, tmp_data,
							     max_bus_size);
...
void hx83112a_nf_diag_register_set(uint8_t diag_command)
{
	...
	hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
	TPD_INFO(
		"%s: tmp_data[3] = 0x%02X, tmp_data[2] = 0x%02X, tmp_data[1] = 0x%02X, tmp_data[0] = 0x%02X!\n",
...
bool hx83112a_nf_wait_wip(int Timing)
{
	...
	do {
		...
		if (in_buffer[0] != 0x00 || in_buffer[1] != 0x00 ||
			...
			TPD_INFO(
				"%s:Wait wip retry_cnt:%d, buffer[0]=%d, buffer[1]=%d, buffer[2]=%d, buffer[3]=%d \n",
...
void hx83112a_nf_sense_on(uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		do {
			...
			tmp_data[1] = 0x00;
			tmp_data[0] = 0x53;
			hx83112a_nf_register_write(tmp_addr, 4, tmp_data,
						   false);
...
{
	hx83112a_nf_register_write(hx83112a_nf_pzf_op->addr_system_reset, 4,
				   hx83112a_nf_pzf_op->data_system_reset,
				   false);
}
...
int hx83112a_nf_dis_rload_0f(int disable)
{
	...
	do {
		hx83112a_nf_flash_write_burst(
			hx83112a_nf_pzf_op->addr_dis_flash_reload,
			hx83112a_nf_pzf_op->data_dis_flash_reload);
		hx83112a_nf_register_read(
			hx83112a_nf_pzf_op->addr_dis_flash_reload, 4, tmp_data,
			false);
		TPD_DETAIL(
			"Now data: tmp_data[3] = 0x%02X || tmp_data[2] = 0x%02X || tmp_data[1] = 0x%02X || tmp_data[0] = 0x%02X\n",
...
void hx83112a_nf_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	tmp_addr[0] = addr[0];
	TPD_DETAIL(
		"%s, write addr tmp_addr[3] = 0x%2.2X, tmp_addr[2] = 0x%2.2X, tmp_addr[1] = 0x%2.2X, tmp_addr[0] = 0x%2.2X\n",
...
void hx83112a_nf_mcu_write_sram_0f(const struct firmware *fw_entry,
				   ...
				   uint32_t write_len)
{
	...
	if (hx83112a_nf_chip_info->tmp_data == NULL) {
		...
		hx83112a_nf_chip_info->tmp_data = kzalloc(
			sizeof(uint8_t) * firmware_update_space, GFP_KERNEL);
		if (hx83112a_nf_chip_info->tmp_data == NULL) {
			TPD_INFO(
				"%s, alloc hx83112a_nf_chip_info->tmp_data failed\n",
				__func__);
			return;
		}
	...
	for (i = 0; i < (total_read_times); i++) {
        ...
        */
		if (total_size_temp >= max_bus_size) {
			hx83112a_nf_flash_write_burst_length(
				tmp_addr,
				&(hx83112a_nf_chip_info
					  ->tmp_data[start_index +
						     i * max_bus_size]),
				max_bus_size);
			total_size_temp = total_size_temp - max_bus_size;
		} else {
...
int hx83112a_nf_sram_write_crc_check(const struct firmware *fw_entry,
				     uint8_t *addr, int strt_idx, uint32_t len)
{
	int retry = 0;
	int crc = -1;

	do {
		g_hx83112a_nf_core_fp.fp_write_sram_0f(fw_entry, addr, strt_idx,
						       len);
		crc = hx83112a_nf_hw_check_CRC(
			hx83112a_nf_pzf_op->data_sram_start_addr, HX_48K_SZ);
		retry++;
...
static int hx83112a_nf_mcu_Calculate_CRC_with_AP(unsigned char *FW_content,
...
bool hx83112a_nf_parse_bin_cfg_data(const struct firmware *fw_entry)
{
	...
	hx83112a_nf_info_arr =
		kzalloc(part_num * sizeof(struct hx83112a_nf_info), GFP_KERNEL);
	...
	for (i = 0; i < part_num; i++) {
		...
		hx83112a_nf_info_arr[i].cfg_addr =
			hx83112a_nf_info_arr[i].sram_addr[0];
		hx83112a_nf_info_arr[i].cfg_addr +=
			hx83112a_nf_info_arr[i].sram_addr[1] << 8;
		hx83112a_nf_info_arr[i].cfg_addr +=
			hx83112a_nf_info_arr[i].sram_addr[2] << 16;
		hx83112a_nf_info_arr[i].cfg_addr +=
			hx83112a_nf_info_arr[i].sram_addr[3] << 24;
	...
	for (i = 0; i < 4; i++)
		hx83112a_nf_sram_min[i] =
			hx83112a_nf_info_arr[i_min].sram_addr[i];
	hx83112a_nf_cfg_sz = (dsram_max - dsram_base) +
			     hx83112a_nf_info_arr[i_max].write_size;
	hx83112a_nf_cfg_sz = hx83112a_nf_cfg_sz + (hx83112a_nf_cfg_sz % 16);

	TPD_INFO(
		"%s, hx83112a_nf_cfg_sz = %d!, dsram_base = %X, dsram_max = %X\n",
		__func__, hx83112a_nf_cfg_sz, dsram_base, dsram_max);

	if (hx83112a_nf_FW_buf == NULL)
		hx83112a_nf_FW_buf = kzalloc(
			sizeof(unsigned char) * FW_BIN_16K_SZ, GFP_KERNEL);

	for (i = 1; i < part_num; i++)
		memcpy(hx83112a_nf_FW_buf +
			       (hx83112a_nf_info_arr[i].cfg_addr - dsram_base),
		       (unsigned char *)&fw_entry
			       ->data[hx83112a_nf_info_arr[i].fw_addr],
		       hx83112a_nf_info_arr[i].write_size);

	hx83112a_nf_cfg_crc = hx83112a_nf_mcu_Calculate_CRC_with_AP(
		hx83112a_nf_FW_buf, 0, hx83112a_nf_cfg_sz);
...
static int hx83112a_nf_nf_zf_part_info(const struct firmware *fw_entry)
{
	...
	hx83112a_nf_sram_write_crc_check(
		fw_entry, hx83112a_nf_pzf_op->data_sram_start_addr, 0,
		HX_48K_SZ);
	crc = hx83112a_nf_hw_check_CRC(hx83112a_nf_pzf_op->data_sram_start_addr,
				       HX_48K_SZ);
	ret = (crc == 0) ? true : false;
	if (crc != 0)
		TPD_INFO("48k CRC Failed! CRC = %X", crc);
	do {
		hx83112a_nf_mcu_register_write(hx83112a_nf_sram_min,
					       hx83112a_nf_cfg_sz,
					       hx83112a_nf_FW_buf, 0);
		/*hx83112a_nf_register_write(hx83112a_nf_sram_min, hx83112a_nf_cfg_sz, hx83112a_nf_FW_buf, 0); */
		crc = hx83112a_nf_hw_check_CRC(hx83112a_nf_sram_min,
					       hx83112a_nf_cfg_sz);
		if (crc != hx83112a_nf_cfg_crc)
			TPD_INFO(
				"Config CRC FAIL, HW CRC = %X, SW CRC = %X, retry time = %d",
				crc, hx83112a_nf_cfg_crc, retry);
		retry++;
	} while (!ret && retry < 10);
	if (HX83112A_NF_POWERONOF == 1)
		g_hx83112a_nf_core_fp.fp_write_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_mode_switch, 0xC33C,
			4);
	else
		g_hx83112a_nf_core_fp.fp_clean_sram_0f(
			hx83112a_nf_pzf_op->data_mode_switch, 4, 2);
...
fw_reload:
	if (fw_entry == NULL || reload) {
		...
		if (hx83112a_nf_chip_info->g_fw_sta) {
			...
			request_fw_headfile->size =
				hx83112a_nf_chip_info->g_fw_len;
			request_fw_headfile->data =
				hx83112a_nf_chip_info->g_fw_buf;
			tmp_fw_entry = request_fw_headfile;

		} else {
			TPD_INFO(
				"request firmware failed, get from headfile\n");
			if (hx83112a_nf_chip_info->p_firmware_headfile
				    ->firmware_data) {
				request_fw_headfile->size =
					hx83112a_nf_chip_info
						->p_firmware_headfile
				...
				TPD_INFO(
					"firmware_data is NULL! exit firmware update!\n");
				if (request_fw_headfile != NULL) {
	...
	if ((int)tmp_fw_entry->size > HX64K) {
		...
		hx83112a_nf_register_write(
			hx83112a_nf_pzf_op->addr_system_reset, 4,
			hx83112a_nf_pzf_op->data_system_reset, false);
		hx83112a_nf_enter_safe_mode();
		/* first 48K */
		do {
			g_hx83112a_nf_core_fp.fp_write_sram_0f(
				tmp_fw_entry,
				hx83112a_nf_pzf_op->data_sram_start_addr, 0,
				HX_48K_SZ);
			crc = hx83112a_nf_hw_check_CRC(
				hx83112a_nf_pzf_op->data_sram_start_addr,
				HX_48K_SZ);
			...
			temp_addr[1] = 0xBF;
			temp_addr[0] = 0xFC;
			hx83112a_nf_register_read(temp_addr, 4, temp_data,
						  false);

			TPD_DETAIL(
				"%s, 48K LAST 4 BYTES: data[3] = 0x%02X, data[2] = 0x%02X, data[1] = 0x%02X, data[0] = 0x%02X\n",
			...
			retry++;
		} while (((temp_data[3] == 0 && temp_data[2] == 0 &&
			   temp_data[1] == 0 && temp_data[0] == 0 &&
			   retry < 80) ||
			  (crc != 0 && retry < 30)) &&
			 !(hx83112a_nf_chip_info->using_headfile && retry < 3));
		...
		if (HX83112A_NF_POWERONOF == 1) {
			...
			do {
				g_hx83112a_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112a_nf_pzf_op->data_cfg_info,
					0xC000, 128); //132
				crc = hx83112a_nf_hw_check_CRC(
					hx83112a_nf_pzf_op->data_cfg_info, 128);
				if (crc == 0) {
					TPD_DETAIL(
					...
					TPD_INFO(
						"%s, config info fail in %d time !\n",
						__func__, retry);
				}
				retry++;
			} while ((crc != 0 && retry < 30) ||
				 (hx83112a_nf_chip_info->using_headfile &&
				  retry < 15));
			...
			g_hx83112a_nf_core_fp.fp_clean_sram_0f(
				hx83112a_nf_pzf_op->data_cfg_info, 128, 2);
		}
		/*FW config*/
		if (HX83112A_NF_POWERONOF == 1) {
			retry = 0;
			do {
				g_hx83112a_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112a_nf_pzf_op->data_fw_cfg_p1,
					0xC100, 528); //482
				crc = hx83112a_nf_hw_check_CRC(
					hx83112a_nf_pzf_op->data_fw_cfg_p1,
					528);
			...
			g_hx83112a_nf_core_fp.fp_clean_sram_0f(
				hx83112a_nf_pzf_op->data_fw_cfg_p1, 528, 1);
		}
		...
		if (HX83112A_NF_POWERONOF == 1) {
			...
			do {
				g_hx83112a_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112a_nf_pzf_op->data_fw_cfg_p3,
					0xCA00, 128);
				crc = hx83112a_nf_hw_check_CRC(
					hx83112a_nf_pzf_op->data_fw_cfg_p3,
					128);
			...
			g_hx83112a_nf_core_fp.fp_clean_sram_0f(
				hx83112a_nf_pzf_op->data_fw_cfg_p3, 128, 1);
		}
		...
		if (HX83112A_NF_POWERONOF == 1) {
			...
			do {
				g_hx83112a_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112a_nf_pzf_op->data_adc_cfg_1,
					0xD640, 1200);
				crc = hx83112a_nf_hw_check_CRC(
					hx83112a_nf_pzf_op->data_adc_cfg_1,
					1200);
			...
			g_hx83112a_nf_core_fp.fp_clean_sram_0f(
				hx83112a_nf_pzf_op->data_adc_cfg_1, 1200, 2);
		}
		...
		if (HX83112A_NF_POWERONOF == 1) {
			...
			do {
				g_hx83112a_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112a_nf_pzf_op->data_adc_cfg_2,
					0xD320, 800);
				crc = hx83112a_nf_hw_check_CRC(
					hx83112a_nf_pzf_op->data_adc_cfg_2,
					800);
			...
			g_hx83112a_nf_core_fp.fp_clean_sram_0f(
				hx83112a_nf_pzf_op->data_adc_cfg_2, 800, 2);
		}
		...
		if (HX83112A_NF_POWERONOF == 1) {
			...
			do {
				g_hx83112a_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112a_nf_pzf_op->data_map_table,
					0xE000, 1536);
				crc = hx83112a_nf_hw_check_CRC(
					hx83112a_nf_pzf_op->data_map_table,
					1536);
				if (crc == 0) {
					TPD_DETAIL(
						"%s, mapping table ok in %d time \n",
						__func__, retry);
					break;
				} else {
					TPD_INFO(
						"%s, mapping table fail in %d time !\n",
						__func__, retry);
				}
				retry++;
			} while ((crc != 0 && retry < 30) ||
			...
			g_hx83112a_nf_core_fp.fp_clean_sram_0f(
				hx83112a_nf_pzf_op->data_map_table, 1536, 2);
		}
	}
	...
	if (HX83112A_NF_POWERONOF == 1) {
		g_hx83112a_nf_core_fp.fp_write_sram_0f(
			tmp_fw_entry, hx83112a_nf_pzf_op->data_mode_switch,
			0xC30C, 4);
	} else {
		g_hx83112a_nf_core_fp.fp_clean_sram_0f(
			hx83112a_nf_pzf_op->data_mode_switch, 4, 2);
	...
	hx83112a_nf_fw_check(hx83112a_nf_pri_ts->chip_data,
			     &hx83112a_nf_pri_ts->resolution_info,
			     &hx83112a_nf_pri_ts->panel_data);
...
int hx83112a_nf_0f_op_file_dirly(char *file_name)
{
	...
	/* trigger reset */
	hx83112a_nf_resetgpio_set(hx83112a_nf_chip_info->hw_res,
				  false); // reset gpio
	hx83112a_nf_resetgpio_set(hx83112a_nf_chip_info->hw_res,
				  true); // reset gpio
...
void hx83112a_nf_mcu_0f_operation(struct work_struct *work)
{
...
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
	if (hx83112a_nf_pri_ts->boot_mode != RECOVERY_BOOT &&
	    !is_oem_unlocked())
#else
	if (hx83112a_nf_pri_ts->boot_mode != MSM_BOOT_MODE__RECOVERY &&
	    !is_oem_unlocked())
#endif
	{
		TPD_INFO("file name = %s\n",
	...
	if (hx83112a_nf_chip_info->debug_mode_sta) {
		if (hx83112a_nf_pri_ts->apk_op &&
		    hx83112a_nf_pri_ts->apk_op->apk_debug_set) {
			hx83112a_nf_pri_ts->apk_op->apk_debug_set(
				hx83112a_nf_pri_ts->chip_data, true);
		}
	}
...
#ifdef HX_0F_DEBUG
void hx83112a_nf_mcu_read_sram_0f(const struct firmware *fw_entry,
				  uint8_t *addr, int start_index, int read_len)
{
	...
	tmp_addr[0] = addr[0];
	TPD_INFO(
		"%s, read addr tmp_addr[3] = 0x%2.2X, tmp_addr[2] = 0x%2.2X, tmp_addr[1] = 0x%2.2X, tmp_addr[0] = 0x%2.2X\n",
		__func__, tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0]);
		...
		for (i = 0; i < read_len; i++, j++) {
			if (not_same_buff[i] == 1) {
				TPD_INFO("bin = [%d] 0x%2.2X\n", i,
					 hx83112a_nf_chip_info->g_fw_buf[j]);
			}
		}
		for (i = 0; i < read_len; i++, j++) {
			if (not_same_buff[i] == 1) {
...
void hx83112a_nf_mcu_read_all_sram(uint8_t *addr, int read_len)
{
    ...
    */
	...
	TPD_INFO(
		"%s, NOW addr tmp_addr[3] = 0x%2.2X, tmp_addr[2] = 0x%2.2X, tmp_addr[1] = 0x%2.2X, tmp_addr[0] = 0x%2.2X\n",
		__func__, tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0]);
...
void hx83112a_nf_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	...
	TPD_INFO("%s, Entering \n", __func__);
	if (type == 0) { /* first 48K */
		g_hx83112a_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_sram_start_addr, 0,
			HX_48K_SZ);
		g_hx83112a_nf_core_fp.fp_read_all_sram(tmp_addr, 0xC000);
	} else { /*last 16k*/
		g_hx83112a_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_cfg_info, 0xC000,
			132);
		g_hx83112a_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_fw_cfg, 0xC0FE, 512);
		g_hx83112a_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_adc_cfg_1, 0xD000,
			376);
		g_hx83112a_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_adc_cfg_2, 0xD178,
			376);
		g_hx83112a_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112a_nf_pzf_op->data_adc_cfg_3, 0xD000,
			376);
		g_hx83112a_nf_core_fp.fp_read_all_sram(
			hx83112a_nf_pzf_op->data_sram_clean, HX_32K_SZ);
	}
	TPD_INFO("%s, END \n", __func__);
}
...
void hx83112a_nf_mcu_0f_operation_check(int type)
{
	...
	TPD_INFO("first 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 hx83112a_nf_chip_info->g_fw_buf[0],
		 hx83112a_nf_chip_info->g_fw_buf[1],
		 hx83112a_nf_chip_info->g_fw_buf[2],
		 hx83112a_nf_chip_info->g_fw_buf[3]);
	TPD_INFO("next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 hx83112a_nf_chip_info->g_fw_buf[4],
		 hx83112a_nf_chip_info->g_fw_buf[5],
		 hx83112a_nf_chip_info->g_fw_buf[6],
		 hx83112a_nf_chip_info->g_fw_buf[7]);
	TPD_INFO("and next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 hx83112a_nf_chip_info->g_fw_buf[8],
		 hx83112a_nf_chip_info->g_fw_buf[9],
		 hx83112a_nf_chip_info->g_fw_buf[10],
		 hx83112a_nf_chip_info->g_fw_buf[11]);
...
{
	hx83112a_nf_pzf_op =
		kzalloc(sizeof(struct hx83112a_nf_operation), GFP_KERNEL);
	...
	g_hx83112a_nf_core_fp.fp_firmware_update_0f =
		hx83112a_nf_mcu_firmware_update_0f;
	g_hx83112a_nf_core_fp.fp_0f_operation = hx83112a_nf_mcu_0f_operation;
	g_hx83112a_nf_core_fp.fp_0f_operation_dirly =
		hx83112a_nf_mcu_0f_operation_dirly;
	g_hx83112a_nf_core_fp.fp_0f_op_file_dirly =
		hx83112a_nf_0f_op_file_dirly;
#ifdef HX_0F_DEBUG
	...
	g_hx83112a_nf_core_fp.fp_firmware_read_0f =
		hx83112a_nf_mcu_firmware_read_0f;
	g_hx83112a_nf_core_fp.fp_0f_operation_check =
		hx83112a_nf_mcu_0f_operation_check;
#endif

	hx83112a_nf_in_parse_assign_cmd(
		zf_addr_dis_flash_reload,
		hx83112a_nf_pzf_op->addr_dis_flash_reload,
		sizeof(hx83112a_nf_pzf_op->addr_dis_flash_reload));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_dis_flash_reload,
		hx83112a_nf_pzf_op->data_dis_flash_reload,
		sizeof(hx83112a_nf_pzf_op->data_dis_flash_reload));
	hx83112a_nf_in_parse_assign_cmd(
		zf_addr_system_reset, hx83112a_nf_pzf_op->addr_system_reset,
		sizeof(hx83112a_nf_pzf_op->addr_system_reset));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_system_reset, hx83112a_nf_pzf_op->data_system_reset,
		sizeof(hx83112a_nf_pzf_op->data_system_reset));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_sram_start_addr,
		hx83112a_nf_pzf_op->data_sram_start_addr,
		sizeof(hx83112a_nf_pzf_op->data_sram_start_addr));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_sram_clean, hx83112a_nf_pzf_op->data_sram_clean,
		sizeof(hx83112a_nf_pzf_op->data_sram_clean));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_cfg_info, hx83112a_nf_pzf_op->data_cfg_info,
		sizeof(hx83112a_nf_pzf_op->data_cfg_info));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_fw_cfg_p1, hx83112a_nf_pzf_op->data_fw_cfg_p1,
		sizeof(hx83112a_nf_pzf_op->data_fw_cfg_p1));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_fw_cfg_p2, hx83112a_nf_pzf_op->data_fw_cfg_p2,
		sizeof(hx83112a_nf_pzf_op->data_fw_cfg_p2));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_fw_cfg_p3, hx83112a_nf_pzf_op->data_fw_cfg_p3,
		sizeof(hx83112a_nf_pzf_op->data_fw_cfg_p3));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_adc_cfg_1, hx83112a_nf_pzf_op->data_adc_cfg_1,
		sizeof(hx83112a_nf_pzf_op->data_adc_cfg_1));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_adc_cfg_2, hx83112a_nf_pzf_op->data_adc_cfg_2,
		sizeof(hx83112a_nf_pzf_op->data_adc_cfg_2));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_adc_cfg_3, hx83112a_nf_pzf_op->data_adc_cfg_3,
		sizeof(hx83112a_nf_pzf_op->data_adc_cfg_3));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_map_table, hx83112a_nf_pzf_op->data_map_table,
		sizeof(hx83112a_nf_pzf_op->data_map_table));
	hx83112a_nf_in_parse_assign_cmd(
		zf_data_mode_switch, hx83112a_nf_pzf_op->data_mode_switch,
		sizeof(hx83112a_nf_pzf_op->data_mode_switch));
...
bool hx83112a_nf_ic_package_check(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (1) {
			...
			g_i_FW_VER =
				i_CTPM_FW[HX83112A_NF_FW_VER_MAJ_FLASH_ADDR]
					<< 8 |
				i_CTPM_FW[HX83112A_NF_FW_VER_MIN_FLASH_ADDR];
			g_i_CFG_VER =
				i_CTPM_FW[HX83112A_NF_CFG_VER_MAJ_FLASH_ADDR]
					<< 8 |
				i_CTPM_FW[HX83112A_NF_CFG_VER_MIN_FLASH_ADDR];
			g_i_CID_MAJ =
				i_CTPM_FW[HX83112A_NF_CID_VER_MAJ_FLASH_ADDR];
			g_i_CID_MIN =
				i_CTPM_FW[HX83112A_NF_CID_VER_MIN_FLASH_ADDR];
#endif
...
static void hx83112a_nf_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		cmd[3] =
			0x10; // oppo fw id bin address : 0xc014   , 49172    Tp ic address : 0x 10007014
		cmd[2] = 0x00;
		cmd[1] = 0x7f;
		cmd[0] = 0x00;
	...
	TPD_INFO(
		"%s : data[0] = 0x%2.2X, data[1] = 0x%2.2X, data[2] = 0x%2.2X, data[3] = 0x%2.2X\n",
...
void hx83112a_nf_read_OPPO_FW_ver(struct chip_data_hx83112a_nf *chip_info)
{
	...
	cmd[3] =
		0x10; // oppo fw id bin address : 0xc014    Tp ic address : 0x 10007014
	cmd[2] = 0x00;
	cmd[1] = 0x70;
	cmd[0] = 0x14;
	hx83112a_nf_register_read(cmd, 4, data, false);
...
int hx83112a_nf_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
	...
	hx83112a_nf_touch_data->touch_all_size =
		128; //hx83112a_nf_get_touch_data_size();
	...
	if ((max_touch_point % 4) == 0)
		...
		HX83112A_NF_TOUCH_INFO_POINT_CNT +=
			((max_touch_point / 4) + 1) * 4;
	...
	if (hx83112a_nf_touch_data->raw_cnt_rmd != 0x00) { //more than 4 fingers
		hx83112a_nf_touch_data->rawdata_size = hx83112a_nf_cal_data_len(
			hx83112a_nf_touch_data->raw_cnt_rmd, max_touch_point,
			hx83112a_nf_touch_data->raw_cnt_max);
		hx83112a_nf_touch_data->touch_info_size =
			(max_touch_point + hx83112a_nf_touch_data->raw_cnt_max +
			 2) *
			4;
	...
	if ((tx_num * rx_num + tx_num + rx_num) %
		    hx83112a_nf_touch_data->rawdata_size ==
	    0) {
		hx83112a_nf_touch_data->rawdata_frame_size =
	...
	TPD_INFO("%s: rawdata_frame_size = %d ", __func__,
		 hx83112a_nf_touch_data->rawdata_frame_size);
	TPD_INFO(
		"%s: max_touch_point:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d, g_hx_rawdata_size:%d, hx83112a_nf_touch_data->touch_info_size:%d\n",
		__func__, max_touch_point, hx83112a_nf_touch_data->raw_cnt_max,
		hx83112a_nf_touch_data->raw_cnt_rmd,
		hx83112a_nf_touch_data->rawdata_size,
		hx83112a_nf_touch_data->touch_info_size);

	hx83112a_nf_touch_data->hx_coord_buf = kzalloc(
		sizeof(uint8_t) * (hx83112a_nf_touch_data->touch_info_size),
		GFP_KERNEL);
	if (hx83112a_nf_touch_data->hx_coord_buf == NULL) {
		goto mem_alloc_fail;
	}

	hx83112a_nf_touch_data->diag_mutual =
	...
	hx83112a_nf_touch_data->hx_rawdata_buf = kzalloc(
		sizeof(uint8_t) * (hx83112a_nf_touch_data->touch_all_size -
				   hx83112a_nf_touch_data->touch_info_size),
		GFP_KERNEL);
	...
	//#if defined(HX_SMART_WAKEUP)
	hx83112a_nf_touch_data->hx_event_buf =
		kzalloc(sizeof(uint8_t) * (hx83112a_nf_touch_data->event_size),
			GFP_KERNEL);
...
void hx83112a_nf_esd_hw_reset(struct chip_data_hx83112a_nf *chip_info)
{
	...
	do {
		hx83112a_nf_resetgpio_set(chip_info->hw_res,
					  true); // reset gpio
		hx83112a_nf_resetgpio_set(chip_info->hw_res,
					  false); // reset gpio
		hx83112a_nf_resetgpio_set(chip_info->hw_res,
		...
		load_fw_times--;
		g_hx83112a_nf_core_fp.fp_0f_operation_dirly();
		ret = g_hx83112a_nf_core_fp.fp_reload_disable(
...
int hx83112a_nf_checksum_cal(struct chip_data_hx83112a_nf *chip_info,
			     uint8_t *buf, int ts_status)
{
	...
	for (loop_i = 0; loop_i < length; loop_i++) {
		...
		if (ts_status == HX_REPORT_COORD) {
			...
			if (hx_EB_event == length) {
				...
				TPD_INFO(
					"[HIMAX TP MSG]: ESD event checked - ALL 0xEB.\n");
			} else if (hx_EC_event == length) {
				hx_esd_event = length;
				hx83112a_nf_EC_event_flag++;
				TPD_INFO(
					"[HIMAX TP MSG]: ESD event checked - ALL 0xEC.\n");
	...
	if (ts_status == HX_REPORT_COORD) {
		...
		if (hx_esd_event == length || hx_zero_event == length) {
			shaking_ret = hx83112a_nf_ic_esd_recovery(
			...
			TPD_INFO(
				"[HX83112A_NF_ESD_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto checksum_fail;
			...
			TPD_INFO(
				"[HX83112A_NF_HW_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto ready_to_serve;
		}
	}
...
void hx83112a_nf_log_touch_data(
	uint8_t *buf, struct hx83112a_nf_report_data *hx83112a_nf_touch_data)
{
	int loop_i = 0;
	int print_size = 0;
...
void hx83112a_nf_idle_mode(int disable)
{
	...
	do {
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s:After turn ON/OFF IDLE Mode [0] = 0x%02X, [1] = 0x%02X, [2] = 0x%02X, [3] = 0x%02X\n",
...
int hx83112a_nf_test_data_pop_out(struct chip_data_hx83112a_nf *chip_info,
				  ...
				  char *filepath)
{
	...
	/*project Info*/
	snprintf(
		g_project_test_info_log, 118,
		"Project_name: %s%d\nFW_ID: %8X\nFW_Ver: %4X\nPanel Info: TX_Num=%d RX_Num=%d\nTest stage: Mobile\n",
		project_name_log, get_project(), chip_info->fw_id,
		chip_info->fw_ver, chip_info->hw_res->TX_NUM,
	...
	if (raw_file == NULL)
		raw_file = file_open_name(vts_name, O_TRUNC | O_CREAT | O_RDWR,
					  0660);
	...
	vfs_write(raw_file, rslt_buf,
		  hx83112a_nf_1kind_raw_size * HX83112A_NF_CRITERIA_ITEM *
			  sizeof(char),
		  &pos);
	if (raw_file != NULL)
...
int hx83112a_nf_test_data_get(struct chip_data_hx83112a_nf *chip_info,
			      uint32_t RAW[], char *start_log, char *result,
			      int now_item)
{
	uint32_t i;
	...
	memcpy(&hx83112a_nf_rslt_data[hx83112a_nf_rslt_data_len], testdata,
	       len);
	hx83112a_nf_rslt_data_len += len;
	TPD_INFO("%s: hx83112a_nf_rslt_data_len=%d!\n", __func__,
		 hx83112a_nf_rslt_data_len);
...
int hx83112a_nf_get_rawdata(struct chip_data_hx83112a_nf *chip_info,
			    uint32_t *RAW, uint32_t datalen)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	uint8_t *tmp_rawdata;
...
uint32_t hx83112a_nf_check_mode(uint8_t checktype)
{
	...
	hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
	TPD_INFO(
		"%s: hx83112a_nf_wait_sorting_mode, tmp_data[0]=%x, tmp_data[1]=%x\n",
		__func__, tmp_data[0], tmp_data[1]);
...
uint32_t hx83112a_nf_wait_sorting_mode(uint8_t checktype)
{
	...
	do {
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 0x900000A8, tmp_data[0]=%x, tmp_data[1]=%x, tmp_data[2]=%x, tmp_data[3]=%x \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 0x900000E4, tmp_data[0]=%x, tmp_data[1]=%x, tmp_data[2]=%x, tmp_data[3]=%x \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 0x900000F8, tmp_data[0]=%x, tmp_data[1]=%x, tmp_data[2]=%x, tmp_data[3]=%x \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
...
int hx83112a_nf_mpTestFunc(struct chip_data_hx83112a_nf *chip_info,
			   uint8_t checktype, uint32_t datalen)
{
	uint8_t tmp_addr[4] = { 0 };
	uint8_t tmp_data[4] = { 0 };
	...
	if (ret) {
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 900000A8: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X, \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 10007F40: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X, \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 10000000: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X, \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 10007F04: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X, \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s: 800204B4: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X, \n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
	...
	case HIMAX_INSPECTION_OPEN:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (hx83112a_nf_isRead_csv == false) {
				...
				if (RAW[i] > hx83112a_nf_inspection_criteria
						     [IDX_OPENMAX][i] ||
				    RAW[i] < hx83112a_nf_inspection_criteria
						     [IDX_OPENMIN][i]) {
					TPD_INFO("%s: open test FAIL\n",
						 __func__);
					ret = RESULT_ERR;
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (hx83112a_nf_isRead_csv == false) {
				if (RAW[i] > CRITERIA_LPWUG_RAWDATA_MAX ||
					...
					TPD_INFO(
						"%s: HIMAX_INSPECTION_LPWUG_RAWDATA FAIL\n",
						__func__);
					ret = THP_AFE_INSPECT_ERAW;
				}
			} else {
				if (RAW[i] >
					    hx83112a_nf_inspection_criteria
	...
	case HIMAX_INSPECTION_LPWUG_NOISE:
		if (hx83112a_nf_isBD12proj || hx83112a_nf_isEA006proj) {
			for (i = 0; i < (chip_info->hw_res->TX_NUM *
				...
				if (hx83112a_nf_isRead_csv == false) {
					if (RAW[i] > LPWUG_NOISE_MAX ||
						...
						TPD_INFO(
							"%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n",
							__func__);
			...
			if (hx83112a_nf_isRead_csv == false) {
				...
				if (weight > LPWUG_NOISEMAX ||
				    weight < hx83112a_nf_inspection_criteria
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (hx83112a_nf_isRead_csv == false) {
				if (RAW[i] > CRITERIA_LPWUG_IDLE_RAWDATA_MAX ||
					...
					TPD_INFO(
						"%s: HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA FAIL\n",
						__func__);
					ret = THP_AFE_INSPECT_ERAW;
				}
			} else {
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (hx83112a_nf_isRead_csv == false) {
				if (RAW[i] > LPWUG_IDLE_NOISE_MAX ||
					...
					TPD_INFO(
						"%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE FAIL\n",
						__func__);
					ret = THP_AFE_INSPECT_ENOISE;
				}
			} else {
	...
	hx83112a_nf_test_data_get(chip_info, RAW, start_log, rslt_log,
				  checktype);
...
int hx83112a_nf_find_crtra_id(char *input)
{
	...
	for (i = 0; i < HX83112A_NF_CRITERIA_SIZE; i++) {
		if (hx83112a_nf_diff_str(hx83112a_nf_inspt_crtra_name[i],
					 input) == 0) {
			result = i;
...
int hx83112a_nf_print_crtra_after_parsing(
	...
	for (i = 0; i < HX83112A_NF_CRITERIA_SIZE; i++) {
		...
		if (hx83112a_nf_inspt_crtra_flag[i] == 1) {
			for (j = 0; j < all_mut_len; j++) {
				TPD_DEBUG(
					"%d, ",
					hx83112a_nf_inspection_criteria[i][j]);
				if (j % 16 == 15)
					TPD_DEBUG("\n");
			}
		} else {
...
static int hx_get_crtra_by_name(struct chip_data_hx83112a_nf *chip_info,
...
static int
hx83112a_nf_parse_criteria_file(struct chip_data_hx83112a_nf *chip_info)
{
	int err = THP_AFE_INSPECT_OK;
	...
	if (result == NULL) {
		...
		if (result != NULL) {
			for (i = 0; i < data_size; i++) {
				...
				if (result[i] == NULL) {
					TPD_INFO(
						"%s: rst_arr Memory allocation falied!\n",
						__func__);
...
static int
hx83112a_nf_self_test_data_init(struct chip_data_hx83112a_nf *chip_info)
{
	int ret = THP_AFE_INSPECT_OK;
	int i = 0;

	/* get test item and its items of criteria*/
	HX83112A_NF_CRITERIA_ITEM =
		hx83112a_nf_get_size_str_arr(hx83112a_nf_inspection_mode);
	HX83112A_NF_CRITERIA_SIZE =
		hx83112a_nf_get_size_str_arr(hx83112a_nf_inspt_crtra_name);
	TPD_INFO(
		"There is %d HX83112A_NF_CRITERIA_ITEM and %d HX83112A_NF_CRITERIA_SIZE\n",
		HX83112A_NF_CRITERIA_ITEM, HX83112A_NF_CRITERIA_SIZE);

	/* init criteria data*/
	hx83112a_nf_inspt_crtra_flag =
		kzalloc(HX83112A_NF_CRITERIA_SIZE * sizeof(int), GFP_KERNEL);
	hx83112a_nf_inspection_criteria =
		kzalloc(sizeof(int *) * HX83112A_NF_CRITERIA_SIZE, GFP_KERNEL);
	if (hx83112a_nf_inspt_crtra_flag == NULL ||
...
static void hx83112a_nf_black_screen_test(void *chip_data, char *message)
{
	...
	char *g_Company_info_log;
	struct chip_data_hx83112a_nf *chip_info =
		(struct chip_data_hx83112a_nf *)chip_data;
	int i = 0;
	...
	if (hx83112a_nf_rslt_data == NULL) {
		...
		hx83112a_nf_rslt_data = kcalloc(
			hx83112a_nf_1kind_raw_size * HX83112A_NF_CRITERIA_ITEM,
			sizeof(char), GFP_KERNEL);
		if (!hx83112a_nf_rslt_data) {
			TPD_INFO(
				"%s:%d hx83112a_nf_rslt_data kzalloc buf error\n",
				__func__, __LINE__);
			goto RET_OUT;
		}
	} else {
		memset(hx83112a_nf_rslt_data, 0x00,
		       hx83112a_nf_1kind_raw_size * HX83112A_NF_CRITERIA_ITEM *
			       sizeof(char));
	}
	/*Init Log Data */
	...
	snprintf(buf, 128, "6. MP_LPWUG_TEST_RAW: %s\n",
		 error ? "Error" : "Ok");
	hx83112a_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83112a_nf_fail_write_count, 15,
			 "test Item:\n");
	hx83112a_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83112a_nf_fail_write_count, 32,
	...
	hx83112a_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83112a_nf_fail_write_count, 22,
			 "Final_result: %s\n", error_num ? "Fail" : "Pass");
	if (error) {
		snprintf(hx83112a_nf_file_path_NG,
			 (int)(strlen(HX_GES_RSLT_OUT_PATH_NG) +
			       strlen(g_file_name_NG) + 1),
			 "%s%s", HX_GES_RSLT_OUT_PATH_NG, g_file_name_NG);
		hx83112a_nf_test_data_pop_out(chip_info, g_Test_list_log,
					      g_Company_info_log,
					      g_project_test_info_log,
					      hx83112a_nf_rslt_data,
					      hx83112a_nf_file_path_NG);
	} else {
		snprintf(hx83112a_nf_file_path_OK,
			 (int)(strlen(HX_GES_RSLT_OUT_PATH_OK) +
...
RET_OUT:
	if (hx83112a_nf_inspection_criteria != NULL) {
		...
		hx83112a_nf_inspection_criteria = NULL;
		TPD_INFO(
			"Now it have free the hx83112a_nf_inspection_criteria!\n");
	} else {
...
int hx83112a_nf_chip_self_test(struct seq_file *s,
			       struct chip_data_hx83112a_nf *chip_info,
			       char *g_Test_list_log)
{
	int error = 0;
...
void hx83112a_nf_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size;
		...
		for (j = 0; j < 5; j++) {
			...
			if (hx83112a_nf_bus_write(0x00, program_length + 4,
						  buring_data) < 0) {
				TPD_INFO("%s: i2c access fail!\n", __func__);
				return;
			}
...
int hx83112a_nf_fts_ctpm_fw_upgrade_with_sys_fs_64k(
					     ...
					     size_t len, loff_t *pos)
{
	...
	if (!HX83112A_NF_HX_PROC_SEND_FLAG) {
		...
		TPD_INFO("hx83112a_nf_register_show: %02X, %02X, %02X, %02X\n",
			 hx83112a_nf_register_command[3],
			 hx83112a_nf_register_command[2],
			 hx83112a_nf_register_command[1],
			 hx83112a_nf_register_command[0]);

		hx83112a_nf_register_read(hx83112a_nf_register_command,
					  max_bus_size, data,
					  hx83112a_nf_cfg_flag);
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' &&
		...
		if (buf[0] == 'r') {
			...
			if (!kstrtoul(buf_tmp, 16, &result)) {
				for (loop_i = 0;
				     loop_i < hx83112a_nf_byte_length;
				     loop_i++) {
					hx83112a_nf_register_command[loop_i] =
			...
			if (count < 3) {
				...
				hx83112a_nf_register_write(
					hx83112a_nf_register_command,
					hx83112a_nf_byte_length, w_data,
					hx83112a_nf_cfg_flag);
			} else {
				hx83112a_nf_byte_length =
					x_pos[1] - x_pos[0] - 2;
				...
				hx83112a_nf_register_write(
					hx83112a_nf_register_command,
					hx83112a_nf_byte_length, &w_data[0],
					hx83112a_nf_cfg_flag);
			}
		} else {
			return len;
...
int hx83112a_nf_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb,
			       uint8_t lb)
{
	int cnt = 0;
	...
	do {
		...
		hx83112a_nf_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO(
			"%s:Now tmp_data[0] = 0x%02X, [1] = 0x%02X, [2] = 0x%02X, [3] = 0x%02X\n",
			__func__, tmp_data[0], tmp_data[1], tmp_data[2],
			tmp_data[3]);
	...
	if (cnt >= 99) {
		TPD_INFO(
			"hx83112a_nf_write_read_reg ERR Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n",
...
void hx83112a_nf_get_DSRAM_data(uint8_t *info_data, uint8_t x_num,
				uint8_t y_num)
{
	int i = 0;
	//int cnt = 0;
	...
	tmp_data[0] = 0xA5;
	fw_run_flag =
		hx83112a_nf_write_read_reg(tmp_addr, tmp_data, 0xA5, 0x5A);
	if (fw_run_flag < 0) {
...
void hx83112a_nf_ts_diag_func(struct chip_data_hx83112a_nf *chip_info,
			      int32_t *mutual_data)
{
	int i = 0;
	int j = 0;
	...
	hx83112a_nf_get_DSRAM_data(info_data, chip_info->hw_res->RX_NUM,
				   chip_info->hw_res->TX_NUM);
...
void hx83112a_nf_diag_parse_raw_data(
	struct hx83112a_nf_report_data *hx83112a_nf_touch_data, int mul_num,
	...
	int32_t *self_data)
{
	...
	if (hx83112a_nf_touch_data->hx_rawdata_buf[0] == 0x3A &&
		...
		index = (hx83112a_nf_touch_data->hx_rawdata_buf[2] - 1) *
			RawDataLen_word;
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) {
				//mutual
				mutual_data[index + i] =
					((int8_t)hx83112a_nf_touch_data
						 ->hx_rawdata_buf[i * 2 + 4 +
								  1]) *
						256 +
					hx83112a_nf_touch_data->hx_rawdata_buf
						[i * 2 +
...
bool hx83112a_nf_diag_check_sum(
	struct hx83112a_nf_report_data
		*hx83112a_nf_touch_data) /*return checksum value  */
{
	uint16_t check_sum_cal = 0;
	int i;
	...
	for (i = 0, check_sum_cal = 0;
	     i < (hx83112a_nf_touch_data->touch_all_size -
		  hx83112a_nf_touch_data->touch_info_size);
	     i = i + 2) {
		check_sum_cal +=
			(hx83112a_nf_touch_data->hx_rawdata_buf[i + 1] * 256 +
			 hx83112a_nf_touch_data->hx_rawdata_buf[i]);
	}
	if (check_sum_cal % 0x10000 != 0) {
					  ...
					  size_t len, loff_t *pos)
{
	...
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	struct chip_data_hx83112a_nf *chip_info =
		(struct chip_data_hx83112a_nf *)ts->chip_data;
	...
	TPD_INFO(" messages       = %s\n"
		 ...
		 " storage_type   = 0x%x\n"
		 " rawdata_type   = 0x%x\n",
		 messages, hx83112a_nf_diag_command, storage_type,
		 rawdata_type);

	if (hx83112a_nf_diag_command > 0 && rawdata_type == 0) {
		TPD_INFO(
			"[Himax]hx83112a_nf_diag_command = 0x%x, storage_type=%d, rawdata_type=%d! Maybe no support!\n",
			hx83112a_nf_diag_command, storage_type, rawdata_type);
		hx83112a_nf_diag_command = 0x00;
	} else {
		TPD_INFO(
			"[Himax]hx83112a_nf_diag_command = 0x%x, storage_type=%d, rawdata_type=%d\n",
			hx83112a_nf_diag_command, storage_type, rawdata_type);
	}
	...
	if (!HX83112A_NF_HX_PROC_SEND_FLAG) {
		...
		//if ((hx83112a_nf_diag_command >= 1 && hx83112a_nf_diag_command <= 7) || dsram_type > 0)
		{
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM *
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx83112a_nf_touch_data->diag_mutual[i] >
				    hx83112a_nf_max_mutual) {
					hx83112a_nf_max_mutual =
						hx83112a_nf_touch_data
							->diag_mutual[i];
				}
				if (hx83112a_nf_touch_data->diag_mutual[i] <
				    hx83112a_nf_min_mutual) {
					hx83112a_nf_min_mutual =
						hx83112a_nf_touch_data
							->diag_mutual[i];
				}
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM +
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx83112a_nf_diag_self[i] >
				    hx83112a_nf_max_self) {
					hx83112a_nf_max_self =
						hx83112a_nf_diag_self[i];
				}
				if (hx83112a_nf_diag_self[i] <
				    hx83112a_nf_min_self) {
					hx83112a_nf_min_self =
						hx83112a_nf_diag_self[i];
				}
			}
...
uint8_t hx83112a_nf_read_DD_status(struct chip_data_hx83112a_nf *chip_info,
				   uint8_t *hx83112a_nf_cmd_set,
				   uint8_t *tmp_data)
{
	int cnt = 0;
	...
	TPD_INFO(
		"%s: hx83112a_nf_cmd_set[0] = 0x%02X, hx83112a_nf_cmd_set[1] = 0x%02X, hx83112a_nf_cmd_set[2] = 0x%02X, hx83112a_nf_cmd_set[3] = 0x%02X\n",
		__func__, hx83112a_nf_cmd_set[0], hx83112a_nf_cmd_set[1],
		hx83112a_nf_cmd_set[2], hx83112a_nf_cmd_set[3]);
					     ...
					     size_t len, loff_t *pos)
{
	...
	if (!HX83112A_NF_HX_PROC_SEND_FLAG) {
		...
		if (hx83112a_nf_mutual_set_flag == 1) {
			if (hx83112a_nf_read_DD_status(chip_info,
						       hx83112a_nf_cmd_set,
						       tmp_data) == NO_ERR) {
				for (loop_i = 0;
				     loop_i < hx83112a_nf_cmd_set[0];
				     loop_i++) {
					if ((loop_i % 8) == 0) {
						ret += snprintf(
							temp_buf + ret,
	...
	if (buf_tmp[2] == 'x' && buf_tmp[6] == 'x' && buf_tmp[10] == 'x') {
		...
		for (i = 3; i < 12; i = i + 4) {
			...
			if (!kstrtoul(buf_tmp2, 16, &result))
				...
				TPD_INFO(
					"String to oul is fail in cnt = %d, buf_tmp2 = %s",
					cnt, buf_tmp2);
			cnt--;
		}
		TPD_INFO(
			"hx83112a_nf_cmd_set[2] = %02X, hx83112a_nf_cmd_set[1] = %02X, hx83112a_nf_cmd_set[0] = %02X\n",
			hx83112a_nf_cmd_set[2], hx83112a_nf_cmd_set[1],
			hx83112a_nf_cmd_set[0]);
	} else
...
int hx83112a_nf_read_FW_status(struct chip_data_hx83112a_nf *chip_info,
			       uint8_t *state_addr, uint8_t *tmp_addr)
{
	...
	if (state_addr[0] == 0x01) {
		...
		req_size = 0x04;
		hx83112a_nf_sense_off();
		hx83112a_nf_register_read(status_addr, req_size, tmp_addr,
					  false);
		hx83112a_nf_sense_on(1);
					     ...
					     size_t len, loff_t *pos)
{
	...
	if (!HX83112A_NF_HX_PROC_SEND_FLAG) {
		...
		if (hx83112a_nf_read_FW_status(chip_info, hx83112a_nf_cmd_set,
					       tmp_data) == NO_ERR) {
			ret += snprintf(temp_buf + ret, len - ret,
					"0x%02X%02X%02X%02X :\t",
					hx83112a_nf_cmd_set[5],
					hx83112a_nf_cmd_set[4],
					hx83112a_nf_cmd_set[3],
					hx83112a_nf_cmd_set[2]);
			for (loop_i = 0; loop_i < hx83112a_nf_cmd_set[1];
			     loop_i++) {
				ret += snprintf(temp_buf + ret, len - ret,
						"%5d\t", tmp_data[loop_i]);
			}
...
static int
hx83112a_nf_configuration_init(struct chip_data_hx83112a_nf *chip_info,
			       bool config)
{
	int ret = 0;
...
int hx83112a_nf_ic_reset(struct chip_data_hx83112a_nf *chip_info,
			 uint8_t loadconfig, uint8_t int_off)
{
	int ret = 0;
	...
	if (chip_info->hw_res->reset_gpio) {
		if (int_off) {
			...
			if (ret < 0) {
				TPD_INFO(
					"%s: hx83112a_nf enable interrupt failed.\n",
					__func__);
				return ret;
			}
		}
		...
		if (loadconfig) {
			...
			if (ret < 0) {
				TPD_INFO(
					"%s: hx83112a_nf configuration init failed.\n",
					__func__);
				return ret;
			}
					...
					struct point_info *points, int max_num)
{
	...
	if (hx83112a_nf_diag_command)
		...
		ret = hx83112a_nf_read_event_stack(
			buf, hx83112a_nf_touch_data->touch_info_size);
	if (!ret) {
	...
	check_sum_cal = hx83112a_nf_checksum_cal(chip_info, buf,
						 ts_status); //????checksum
	...
	if (ts_status == HX_REPORT_COORD) {
		memcpy(hx83112a_nf_touch_data->hx_coord_buf, &buf[0],
		       hx83112a_nf_touch_data->touch_info_size);
		if (buf[hx_state_info_pos] != 0xFF &&
		    buf[hx_state_info_pos + 1] != 0xFF) {
			memcpy(hx83112a_nf_touch_data->hx_state_info,
			...
			memset(hx83112a_nf_touch_data->hx_state_info, 0x00,
			       sizeof(hx83112a_nf_touch_data->hx_state_info));
		}
	}
	if (hx83112a_nf_diag_command) {
		mutual_num =
		...
		self_num =
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
		TPD_INFO(
			"hx83112a_nf_touch_data->touch_all_size= %d hx83112a_nf_touch_data->touch_info_size = %d, %d\n",
			hx83112a_nf_touch_data->touch_all_size,
			hx83112a_nf_touch_data->touch_info_size,
			hx83112a_nf_touch_data->touch_all_size -
				hx83112a_nf_touch_data->touch_info_size);
		memcpy(hx83112a_nf_touch_data->hx_rawdata_buf,
		       &buf[hx83112a_nf_touch_data->touch_info_size],
		       hx83112a_nf_touch_data->touch_all_size -
			       hx83112a_nf_touch_data->touch_info_size);
		...
		hx83112a_nf_diag_parse_raw_data(
			hx83112a_nf_touch_data, mutual_num, self_num,
			hx83112a_nf_diag_command,
			hx83112a_nf_touch_data->diag_mutual,
			hx83112a_nf_diag_self);
	}

	if (hx83112a_nf_touch_data
		    ->hx_coord_buf[HX83112A_NF_TOUCH_INFO_POINT_CNT] ==
	    0xff) //HX83112A_NF_TOUCH_INFO_POINT_CNT buf???????
		hx_point_num = 0;
	else
		hx_point_num =
			hx83112a_nf_touch_data
	...
	for (i = 0; i < 10; i++) {
		x = hx83112a_nf_touch_data->hx_coord_buf[i * 4] << 8 |
		    hx83112a_nf_touch_data->hx_coord_buf[i * 4 + 1];
		y = (hx83112a_nf_touch_data->hx_coord_buf[i * 4 + 2] << 8 |
		     hx83112a_nf_touch_data->hx_coord_buf[i * 4 + 3]);
		z = hx83112a_nf_touch_data->hx_coord_buf[i + 40];
		if (x >= 0 && x <= hx83112a_nf_pri_ts->resolution_info.max_x &&
		    y >= 0 && y <= hx83112a_nf_pri_ts->resolution_info.max_y) {
			points[i].x = x;
			points[i].y = y;
		     ...
		     struct panel_info *panel_data)
{
	...
	if (panel_data->manufacture_info.version)
		sprintf(panel_data->manufacture_info.version, "0x%x-%d",
			panel_data->TP_FW, hx83112a_nf_lcd_vendor);

	return FW_NORMAL;
}

static u8 hx83112a_nf_trigger_reason(void *chip_data, int gesture_enable,
				     int is_suspended)
{
...
void hx83112a_nf_ultra_enter(void)
{
	...
	do {
		...
		TPD_INFO(
			"%s:retry times %d, addr = 0x34, correct 0x11 = current 0x%2.2X\n",
	...
	do {
		...
		TPD_INFO(
			"%s:retry times %d, addr = 0x33, correct 0x33 = current 0x%2.2X\n",
			__func__, rtimes, tmp_data[0]);
		rtimes++;
	...
	do {
		...
		TPD_INFO(
			"%s:retry times %d, addr = 0x34, correct 0x22 = current 0x%2.2X\n",
			__func__, rtimes, tmp_data[0]);
		rtimes++;
	...
	do {
		...
		TPD_INFO(
			"%s:retry times %d, addr = 0x33, correct 0xAA = current 0x%2.2X\n",
			__func__, rtimes, tmp_data[0]);
		rtimes++;
...
static int
hx83112a_nf_enable_black_gesture(struct chip_data_hx83112a_nf *chip_info,
				 bool enable)
{
	int ret = 0;
...
static int
hx83112a_nf_enable_charge_mode(struct chip_data_hx83112a_nf *chip_info,
			       bool enable)
{
	int ret = 0;
	uint8_t tmp_addr[4];
...
/*on = 1:on   0:off */
static int hx83112a_nf_jitter_switch(struct chip_data_hx83112a_nf *chip_info,
				     bool on)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
		...
		do {
			if (rtimes > 10) {
				TPD_INFO(
					...
					__func__);
				TPD_INFO(
					"%s:correct tmp_data[0, 1, 2, 3] = 0x5A, 0xA5, 0x5A, 0xA5\n",
					__func__);
				ret = -1;
				break;
			}
			...
			TPD_INFO(
				"%s:retry times %d, current tmp_data[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
		...
		do {
			if (rtimes > 10) {
				TPD_INFO(
					...
					__func__);
				TPD_INFO(
					"%s:correct tmp_data[0, 1, 2, 3] = 0x00, 0x00, 0x00, 0x00\n",
					__func__);
				ret = -1;
				break;
			}
...
static int
hx83112a_nf_enable_headset_mode(struct chip_data_hx83112a_nf *chip_info,
				bool enable)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	if (ts->headset_pump_support) {
		if (enable) { /* insert headset */
			do {
				...
				tmp_data[1] = 0xA5;
				tmp_data[0] = 0x5A;
				hx83112a_nf_flash_write_burst(tmp_addr,
							      tmp_data);

				hx83112a_nf_register_read(tmp_addr, 4, tmp_data,
							  false);
				TPD_DETAIL(
					"%s:retry times %d, current tmp_data[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
					__func__, rtimes, tmp_data[0],
	...
	if (ts->fw_edge_limit_support) {
		if (mode == 1 ||
			...
			if (LANDSCAPE_SCREEN_270 ==
				...
				do {
					if (rtimes > 10) {
						TPD_INFO(
							"%s:rotative right failed!\n",
							__func__);
						TPD_INFO(
							"%s:correct tmp_data[0, 1, 2, 3] = 0x3A, 0xA3, 0x3A, 0xA3\n",
							__func__);
						ret = -1;
						break;
					}
				...
				do {
					if (rtimes > 10) {
						TPD_INFO(
							"%s:rotative left failed!\n",
							__func__);
						TPD_INFO(
							"%s:correct tmp_data[0, 1, 2, 3] = 0x1A, 0xA1, 0x1A, 0xA1\n",
							__func__);
						ret = -1;
						break;
					}
		...
		if (mode) { //open
			...
			do {
				if (rtimes > 10) {
					TPD_INFO(
						"%s:close edge limit failed!\n",
						__func__);
					TPD_INFO(
						"%s:correct tmp_data[0, 1, 2, 3] = 0x9A, 0xA9, 0x9A, 0xA9\n",
						__func__);
					ret = -1;
					break;
				}
...
static int hx83112a_nf_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_EDGE:
		...
		if (ret < 0) {
			TPD_INFO(
				"%s: hx83112a_nf enable edg & corner limit failed.\n",
				__func__);
			return ret;
		}
					...
					struct gesture_info *gesture)
{
	...
	buf = kzalloc(hx83112a_nf_touch_data->event_size * sizeof(uint8_t),
		      GFP_KERNEL);
	if (!buf) {
	...
	hx83112a_nf_burst_enable(0);
	if (!hx83112a_nf_read_event_stack(buf,
					  hx83112a_nf_touch_data->event_size)) {
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		if (hx83112a_nf_check_point_format == 0) {
			...
			while (i < (gest_len + 1) / 2) {
				if (i == 6) {
					hx83112a_nf_gest_pt_x
						[hx83112a_nf_gest_pt_cnt] =
							buf[GEST_PTLG_ID_LEN +
							    4 + i * 2];
				} else {
					hx83112a_nf_gest_pt_x
						[hx83112a_nf_gest_pt_cnt] =
				...
				hx83112a_nf_gest_pt_y[hx83112a_nf_gest_pt_cnt] =
			...
			if (hx83112a_nf_pri_ts->gesture_buf) {
				...
				hx83112a_nf_pri_ts->gesture_buf[0] =
					gesture_sign;
				hx83112a_nf_pri_ts->gesture_buf[1] = buf[127];

				if (hx83112a_nf_pri_ts->gesture_buf[0] ==
				    0x07) {
					...
					hx83112a_nf_pri_ts->gesture_buf[2] =
						pt_num;
				} else {
					hx83112a_nf_pri_ts->gesture_buf[2] =
						gest_len;
					memcpy(&hx83112a_nf_pri_ts
							->gesture_buf[3],
					       &buf[24], 80);
				}
			}
		...
		if (hx83112a_nf_gest_pt_cnt) {
			...
			gesture->Point_start.x =
				hx83112a_nf_gest_pt_x[0]; /* start x */
			gesture->Point_start.y =
				hx83112a_nf_gest_pt_y[0]; /* start y */
			gesture->Point_end.x =
				hx83112a_nf_gest_pt_x[1]; /* end x */
			gesture->Point_end.y =
				hx83112a_nf_gest_pt_y[1]; /* end y */
				    ...
				    char *g_Test_list_log)
{
	...
	if (eint_count == 10) {
		...
		hx83112a_nf_fail_write_count += snprintf(
			g_Test_list_log + hx83112a_nf_fail_write_count, 45,
...
static void hx83112a_nf_auto_test(struct seq_file *s, void *chip_data,
				  struct syna_testdata *syna_testdata)
{
	int error_count = 0;
	int ret = THP_AFE_INSPECT_OK;
	...
	error_count += hx83112a_nf_int_pin_test(s, chip_info, syna_testdata,
						g_Test_list_log);
	error_count +=
		hx83112a_nf_chip_self_test(s, chip_info, g_Test_list_log);
	...
	if (error_count) {
		snprintf(hx83112a_nf_file_path_NG,
			 (int)(strlen(HX_RSLT_OUT_PATH_NG) +
			       strlen(g_file_name_NG) + 1),
		...
		snprintf(hx83112a_nf_file_path_OK,
			 (int)(strlen(HX_RSLT_OUT_PATH_OK) +
			       strlen(g_file_name_OK) + 1),
...
static fw_update_state
hx83112a_nf_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
...
static int hx83112a_nf_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:HIMAX%06X\nFW_VER:0x%04X\nCH:%dX%d\n",
		       0x83112A, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
		       chip_info->hw_res->RX_NUM);
	if (ret > len) {
		ret = len;
	}
...
static int hx83112a_nf_tp_probe(struct spi_device *spi)
{
	...
	hx83112a_nf_touch_data =
		kzalloc(sizeof(struct hx83112a_nf_report_data), GFP_KERNEL);
	...
	INIT_DELAYED_WORK(&chip_info->work_0f_update,
			  hx83112a_nf_mcu_0f_operation);
	...
	//touch data init
	ret = hx83112a_nf_report_data_init(ts->max_num, ts->hw_res.TX_NUM,
					   ts->hw_res.RX_NUM);
	if (ret) {
		goto err_register_driver;
	}
