	...
	if (limit_data != NULL) {
		for (i = 0; i < limit_data->item_size; i++) {
			...
			TPD_INFO("%s: [%d]Size of =%d\n", __func__, i,
				 (int)sizeof(limit_data->item_name[i]));
...
static ssize_t himax_proc_vendor_read(struct file *file, char *buff, size_t len,
				      loff_t *pos)
{
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	struct himax_proc_operations *syna_ops;
		      ...
		      struct himax_proc_operations *syna_ops)
{
	...
	if (prEntry_himax == NULL) {
		ret = -ENOMEM;
		TPD_INFO(
			"%s: Couldn't create /proc/touchpanel/debug_info/himax proc entry\n",
			__func__);
	}
	...
	prEntry_tmp = proc_create_data("vendor", 0666, prEntry_himax,
