...
static void input_delay_keyup(struct timer_list *t)
{
	// shygosh: Port to 4.19
	struct input_dev *dev = from_timer(dev, t, timer);
	unsigned long flags;
	spin_lock_irqsave(&dev->event_lock, flags);
			  ...
			  unsigned int type, unsigned int code, int *pval)
{
	...
	case EV_KEY:
		...
		#else
			if (!!test_bit(code, dev->key) != !!value) {
				// shygosh: Fix address of 'dev->keyup_data.keyup_timer' will always evaluate to 'true'
				if (value == 0 && (code == KEY_MENU || code == KEY_BACK || code == KEY_HOMEPAGE) &&
				    (&dev->keyup_data.keyup_timer != NULL)) {
					if (code == KEY_MENU)
...
struct input_dev *input_allocate_device(void)
{
	...
	if (dev) {
		...
		// Tong.Han@Bsp.Group.Tp,2017/2/27,Modify for shortcut function.
		// shygosh: Port to 4.19
		timer_setup(&(dev->keyup_data.keyup_timer), NULL, 0);
	#endif
