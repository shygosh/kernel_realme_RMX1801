...
static int get_ant_select_gpio(struct devinfo_data *devinfo_data)
{
	...
	devinfo_data->ant_select_gpio =
		of_get_named_gpio(np, "Hw,ant_select-gpio", 0);

	if (devinfo_data->ant_select_gpio >= 0) {
		if (gpio_is_valid(devinfo_data->ant_select_gpio)) {
			ret = gpio_request(devinfo_data->ant_select_gpio,
					   "ant_select-gpio");
			if (ret) {
...
static int get_hw_opreator_version(struct devinfo_data *devinfo_data)
{
	...
	devinfo_data->hw_operator_gpio_sleep = pinctrl_lookup_state(
		devinfo_data->pinctrl, "hw_operator_gpio_sleep");
...
#endif
	pinctrl_select_state(devinfo_data->pinctrl,
...
sub_mainboard_set:
	...
	case OPPO_16118: {
		...
		if (get_PCB_Version() == HW_VERSION__10)
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "%d-audio-china", get_project());
		else {
			if (id1 == 1 && id2 == 1)
				snprintf(mainboard_info.manufacture,
					 INFO_BUF_LEN, "%d-audio-china",
					 get_project());
			else if (id1 == 0 && id2 == 0)
				snprintf(mainboard_info.manufacture,
					 INFO_BUF_LEN, "%d-audio-oversea",
					 get_project());
			else
	...
	case OPPO_17021: {
		...
		if (id2 == 1)
			snprintf(speaker_mainboard_info.manufacture,
				 INFO_BUF_LEN, "%d-speaker-china",
				 get_project());
		else if (id2 == 0)
			snprintf(speaker_mainboard_info.manufacture,
				 INFO_BUF_LEN, "%d-speaker-oversea",
				 get_project());
		else
			speaker_mainboard_info.manufacture = "sub-UNSPECIFIED";

		register_device_proc("speaker_mainboard",
	...
	case OPPO_18316: {
		pr_err("id1 = %d, id2 = %d\n", id1, id2);
		if ((id1 == 0) && (id2 == 0) &&
		    (get_Modem_Version() == RF_VERSION__13)) {
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "18316-all-match");
		} else if ((id1 == 1) && (id2 == 1) &&
			   (get_Modem_Version() == RF_VERSION__12)) {
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "18315-indian-match");
		} else
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
	...
	case OPPO_18005: {
		pr_err("id1 = %d, id2 = %d\n", id1, id2);
		if ((id1 == 1) && (id2 == 1) &&
		    ((get_Modem_Version() == RF_VERSION__14) ||
		     (get_Modem_Version() == RF_VERSION__15) ||
		     (get_Modem_Version() == RF_VERSION__1B))) {
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "18005-all-match");
		} else
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "%d-%d-%d-%d-unmatch", get_project(),
				 get_Modem_Version(), id1, id2);
	...
	case OPPO_18321: {
		pr_err("id1 = %d, id2 = %d\n", id1, id2);
		if ((id1 == 1) && (id2 == 1) &&
		    ((get_Modem_Version() == RF_VERSION__17) ||
		     (get_Modem_Version() == RF_VERSION__18) ||
		     (get_Modem_Version() == RF_VERSION__19) ||
		     (get_Modem_Version() == RF_VERSION__1A))) {
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "18321-real-indian-match");
		} else
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "%d-%d-%d-%d-unmatch", get_project(),
				 get_Modem_Version(), id1, id2);
	...
	case OPPO_18323: {
		pr_err("id1 = %d, id2 = %d\n", id1, id2);
		if ((id1 == 0) && (id2 == 0) &&
		    (get_Modem_Version() == RF_VERSION__16)) {
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "18323-oppo-oversea-match");
		} else
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "%d-%d-%d-%d-unmatch", get_project(),
				 get_Modem_Version(), id1, id2);
	...
	default: {
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d",
	...
	register_device_proc("audio_mainboard", mainboard_info.version,
...
static void mainboard_verify(struct devinfo_data *devinfo_data)
{
	...
	case HW_VERSION__10:
		...
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-T0",
			 hw_opreator_version);
		//		mainboard_info.manufacture = "SA(SB)";
		break;
	case HW_VERSION__11:
		mainboard_info.version = "11";
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-SA",
			 hw_opreator_version);
	...
	case HW_VERSION__13:
		...
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-SB",
			 hw_opreator_version);
		//        mainboard_info.manufacture = "SE";
		break;
	case HW_VERSION__14:
		mainboard_info.version = "14";
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-SC",
			 hw_opreator_version);
		//		mainboard_info.manufacture = "SF";
		break;
	case HW_VERSION__15:
		mainboard_info.version = "15";
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-(T3-T4)",
			 hw_opreator_version);
		//		mainboard_info.manufacture = "T3-T4";
		break;
	default:
		mainboard_info.version = "UNKOWN";
		snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-UNKOWN",
			 hw_opreator_version);
...
//#ifdef CONFIG_OPPO_VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,modified 2014.4.13 for 14027
...
wlan_resource_set:
	...
	case OPPO_16118: {
		...
		if (id1 == 1 && id2 == 1) {
			...
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "%d-wlan-first", get_project());
		} else if (id1 == 1 && id2 == 0) {
			mainboard_res = MAINBOARD_RESOURCE2;
			snprintf(mainboard_info.manufacture, INFO_BUF_LEN,
				 "%d-wlan-second", get_project());
		} else {
	...
	register_device_proc("wlan_resource", mainboard_info.version,
			     mainboard_info.manufacture);
}
...
static int devinfo_probe(struct platform_device *pdev)
{
	...
	pentry = proc_create("wlan_res", S_IRUGO, NULL,
			     &mainboard_res_proc_fops);
	if (!pentry) {
