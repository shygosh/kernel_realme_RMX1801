...
#define OEM_SEC_ENABLE_ANTIROLLBACK_REG                                        \
	0x78019c /*sdm660
*/
...
#define QFPROM_RAW_SERIAL_NUM                                                  \
	0x00786134 /*different at each platform, please ref boot_images\core\systemdrivers\hwio\scripts\xxx\hwioreg.per
*/
...
static ssize_t expOpera_proc_read(struct file *file, char __user *buf,
				  size_t count, loff_t *off)
{
	char page[256] = { 0 };
	int len = 0;
...
static int __init oppo_project_init(void)
{
	...
	pentry = proc_create("oppoBootmode", S_IRUGO, oppoVersion,
			     &oppoBootmode_proc_fops);
	if (!pentry) {
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
	/*Ziqing.Guo@BSP.Fingerprint.Secure 2017/04/16 Add for distinguishing secureboot stage*/
	pentry = proc_create("secureStage", S_IRUGO, oppoVersion,
			     &secureStage_proc_fops);
	if (!pentry) {
		pr_err("create secureStage proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
#endif
	pentry = proc_create("serialID", S_IRUGO, oppoVersion,
			     &serialID_proc_fops);
	if (!pentry) {
...
static int __init expOpera_init(void)
{
	...
	for (i = 0; substr[i] != ' ' && i < ITEM_LENGTH && substr[i] != '\0';
	     i++) {
		expOpera[i] = substr[i];
	}
	expOpera[i] = '\0';
