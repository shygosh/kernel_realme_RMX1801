...
#define OPPO_CHG_UPDATE_INIT_DELAY                                             \
	round_jiffies_relative(                                                \
		msecs_to_jiffies(500)) /* first run after init 10s */
#define OPPO_CHG_UPDATE_INTERVAL                                               \
	round_jiffies_relative(msecs_to_jiffies(OPPO_CHG_UPDATE_INTERVAL_SEC * \
						1000)) /* update cycle 5s */
...
#define charger_xlog_printk(num, fmt, ...)                                     \
	do {                                                                   \
		if (enable_charger_log >= (int)num) {                          \
			printk(KERN_NOTICE pr_fmt("[OPPO_CHG][%s]" fmt),       \
			       __func__, ##__VA_ARGS__);                       \
		}                                                              \
	} while (0)

void (*synaptics_chg_mode_enable)(
	int enable,
	int is_fast_charge); /*enable  0: no   1: slow  3:  quickly charge*/
			   ...
			   union power_supply_propval *val)
{
	...
	struct oppo_chg_chip *chip =
		container_of(psy, struct oppo_chg_chip, ac_psy);
				...
				const union power_supply_propval *val)
{
	int ret = 0;
	struct oppo_chg_chip *chip =
		container_of(psy, struct oppo_chg_chip, batt_psy);
					    ...
					    loff_t *data)
{
	...
	if (g_charger_chip &&
	    (limit_current <= OPCHG_INPUT_CURRENT_LIMIT_LED_MA)) {
		g_charger_chip->limits.input_current_led_ma = limit_current;
		printk(KERN_EMERG
		       "charging_feature:input_current_led_ma = %d\n",
		       g_charger_chip->limits.input_current_led_ma);
	}
	return len;
}
...
void usb_enum_timer_init(void)
{
	...
	hrtimer_init(&g_charger_chip->usb_enum_hrtimer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
...
void usb_distinguish_timer_init(void)
{
	...
	hrtimer_init(&g_charger_chip->usb_distinguish_hrtimer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	g_charger_chip->usb_distinguish_hrtimer.function =
		usb_distinguish_timerout_interrupt;
}
...
int oppo_chg_parse_dt(struct oppo_chg_chip *chip)
{
	...
	chip->usb_shell_ctrl_gpio.usb_shell_gpio =
		of_get_named_gpio(node, "qcom,usb_shell_ctrl_gpio", 0);
	...
	chip->usb_shell_ctrl =
		of_property_read_bool(node, "qcom,usb_shell_ctrl");

	chip->usb_enum_enable =
		of_property_read_bool(node, "qcom,usb_enum_enable");
	/*hardware init*/
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_vfloat_mv_normalchg",
	...
	charger_xlog_printk(
		...
		chip->suspend_after_full, chip->external_gauge,
...
static void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			    " led_on = %d, current_limit = %d, input = %d\n",
	...
	if ((chip->led_on) &&
		...
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"[BATTERY]Batt_temp STATUS CHANGED, Over Temp\n");
	}
	if (chip->usb_enum_enable) {
		if (chip->usb_enum_type == USB_ENUM_DEFAULT) {
			if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
				if (current_limit > 900)
					current_limit = 900;
				chg_debug(
					"usb_enum default,set 900mA current_limit\n");
			}
...
static void oppo_chg_vfloat_over_check(struct oppo_chg_chip *chip)
{
	...
	if (chip->limits.sw_vfloat_over_protect_enable) {
		if ((chip->batt_volt >=
			...
			if (chip->limits.vfloat_over_counts > VFLOAT_OVER_NUM) {
				...
				chip->chg_ops->float_voltage_write(
					chip, chip->limits.vfloat_sw_set);
			}
		} else {
			chip->limits.vfloat_over_counts = 0;
		}
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"bat_volt:%d, tbatt:%d, counts:%d, sw_vfloat_set:%d\n",
			chip->batt_volt, chip->tbatt_status,
			chip->limits.vfloat_over_counts,
			chip->limits.vfloat_sw_set);
		return;
	}
...
static void oppo_chg_check_rechg_status(struct oppo_chg_chip *chip)
{
	...
	if (!chip->authenticate) {
		recharging_vol =
			chip->limits.non_standard_vfloat_sw_limit - 400;
	}
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			if (chip->usb_enum_enable) {
				if (chip->charger_type !=
					...
					chg_debug(
						" !sdp force to set dcp fist thread,usb_enum\n");
				}
			}
			...
			charger_xlog_printk(CHG_LOG_CRTI,
					    ...
					    chip->charger_type);
			charger_resumed =
				chip->chg_ops->check_charger_resume(chip);
			oppo_chg_turn_on_charging(chip);
			...
			if (charger_resumed == false) {
				...
				oppo_chg_turn_on_charging(chip);
				   ...
				   (chip->usb_enum_type == USB_ENUM_TIMEOUT ||
				    chip->usb_enum_type == USB_ENUM_DETECTED) &&
				   !chip->usb_enum_timeout) {
				oppo_chg_set_input_current_limit(chip);
				chg_debug(
					" usb_enum type:%d, set current again",
					chip->usb_enum_type);
			}
			...
			if (chip->usb_enum_enable &&
				...
				chg_debug(
					" judge whether to disconnect usb,usb_enum type:%d\n",
					chip->usb_enum_type);
			}
		}
	} else {
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	if (ui_soc_cp_flag == 0) {
		...
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"[Battery]oppo_chg_get_battery_data,soc_load = %d,soc = %d\n",
			soc_load, chip->soc);
	}
next:
	return;
...
static void oppo_chg_protection_check(struct oppo_chg_chip *chip)
{
	if (false == oppo_chg_check_tbatt_is_good(chip)) {
		...
		oppo_chg_voter_charging_stop(chip,
...
static void oppo_chg_update_ui_soc(struct oppo_chg_chip *chip)
{
	...
	if (chip->batt_exist && chip->batt_volt < 3300 &&
	    chip->batt_volt > 2500) {
		soc_down_limit = SOC_SYNC_DOWN_RATE_15S;
		vbatt_too_low = true;
		vbatt_lowerthan_3300mv = true;
		charger_xlog_printk(CHG_LOG_CRTI,
				    "batt_volt:%d, vbatt_too_low:%d\n",
	...
	if (chip->charger_exist && chip->batt_exist && chip->batt_full &&
		...
		if (chip->ui_soc != ui_soc_pre) {
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"charging ui_soc:%d,soc:%d,down_limit:%d,up_limit:%d\n",
				chip->ui_soc, chip->soc, soc_down_limit,
				soc_up_limit);
		}
	} else {
		chip->prop_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
...
static void oppo_chg_print_log(struct oppo_chg_chip *chip)
{
	...
	charger_xlog_printk(
		CHG_LOG_CRTI,
		" CHGR[ %d / %d / %d / %d / %d ], BAT[ %d / %d / %d / %d / %d / %d ], GAUGE[ %d / %d / %d / %d / %d / %d / %d ], "
		"STATUS[ 0x%x / %d / %d / %d / %d / 0x%x ], OTHER[ %d / %d / %d / %d / %d ]\n",
		...
		chip->batt_volt, chip->icharging, chip->soc, chip->ui_soc,
		chip->soc_load, chip->batt_rm, chip->vbatt_over,
		chip->chging_over_time, chip->vchg_status, chip->tbatt_status,
		chip->stop_voter, chip->notify_code,
...
static void oppo_chg_ibatt_check_and_set(struct oppo_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
		...
		recharge_volt = chip->limits.temp_normal_vfloat_mv_normalchg -
				chip->limits.recharge_mv;
...
static void oppo_chg_update_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oppo_chg_chip *chip =
		container_of(dwork, struct oppo_chg_chip, update_work);
...
void oppo_set_usb_shell_ctrl(int value)
{
	...
	if (g_charger_chip && g_charger_chip->chg_ops->set_usb_shell_ctrl_val) {
		g_charger_chip->chg_ops->set_usb_shell_ctrl_val(g_charger_chip,
								value);
	}
}
int oppo_get_usb_shell_ctrl(void)
{
	...
	if (g_charger_chip && g_charger_chip->chg_ops->get_usb_shell_ctrl_val) {
		return g_charger_chip->chg_ops->get_usb_shell_ctrl_val(
			g_charger_chip);
	} else {
