				...
				struct mdp_display_commit *data)
{
	...
	if (mfd->panel.type == WRITEBACK_PANEL) {
		...
		// Guoqiang.jiang@PSW.MM.Display.LCD.Stability, 2018/10/15, add for set dither config in screen lock to solve water ripple.
...
int mdss_mdp_overlay_vsync_ctrl(struct msm_fb_data_type *mfd, int en)
{
	...
	if (!ctl->panel_data->panel_info.cont_splash_enabled
		...
		(IS_ENABLED(CONFIG_OPPO_VENDOR_EDIT) ? false :
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
	// Gou Shengjun@PSW.MM.Display.LCD.Feature, 2018/01/03, add for dynamic fps switch
