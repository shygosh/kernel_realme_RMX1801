...
// Guoqiang.Jiang@MultiMedia.Display.LCD.Stability, 2018/10/12, add for Lcd ftm \ project and mmkey
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
// Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/30, add for i2c backlight
...
// Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/12, add for get panel serial number
bool flag_lcd_off = false;
static DEFINE_MUTEX(lcd_mutex);
struct mdss_dsi_ctrl_pdata *gl_ctrl_pdata;

// Zeke.Shi@RM.MM.Display.LCD.Feature, 2018/12/20, add for lcd cabc
static int cabc_lastlevel = 1;

// Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/31, add for close bl for silence and sau mode
extern int lcd_closebl_flag;
// Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/02/14, add for lcd cabc
enum
{
	CABC_CLOSE = 0,
	CABC_LOW_MODE,
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	struct mdss_panel_info *pinfo;
#ifdef CONFIG_OPPO_VENDOR_EDIT
	// Shengjun.Gou@PSW.MM.Display.LCD.Stability, 2017/06/13, add for 1024 level backlight
	int	BL_MSB = 0;
	int	BL_LSB = 0;
#endif
...
#else
	//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/31 add for 1024 level backlight
...
#else
	//Guoqiang.Jiang@PSW.MultiMedia.Display.LCD.Stability, 2018/10/31 add for 1024 level backlight
	if (lcd_vendor == OPPO17011_SAMSUNG_SOFEG01_S_1080P_CMD_PANEL ||
	    lcd_vendor == OPPO17021_SAMSUNG_SOFEG01_S_1080P_CMD_PANEL ||
	    lcd_vendor == OPPO17081_SAMSUNG_AMS596W401_1080P_CMD_PANEL ||
	    lcd_vendor == OPPO18005_SAMSUNG_AMS641RW01_1080P_CMD_PANEL) {
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
//Guoqiang.Jiang@PSW.MM.Driver.feature, 2017/03/17, add for HBM
...
//Guoqiang.Jiang@PSW.MM.Driver.feature, 2017/03/17, add for read panel serial number
...
int panel_serial_number_read(char addr, uint64_t *buf)
{
#define PANEL_REG_READ_LEN 16
	int ret = 0;
	unsigned char read[PANEL_REG_READ_LEN];
	...
	mutex_lock(&lcd_mutex);
	ret = mdss_dsi_panel_cmd_read(gl_ctrl_pdata, addr, 0x00, NULL, read, PANEL_REG_READ_LEN);
	mutex_unlock(&lcd_mutex);
...
//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2017/02/14, add for lcd cabc
...
//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2017/02/18, add for lcd seed
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/30, add for tp black gesture
...
#ifndef CONFIG_OPPO_VENDOR_EDIT
		//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/30, delete for not used
		if (gpio_is_valid(ctrl_pdata->lcd_mode_sel_gpio)) {
			bool out = false;
...
#ifndef CONFIG_OPPO_VENDOR_EDIT
		//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2018/10/30, remove for not used
		if (gpio_is_valid(ctrl_pdata->avdd_en_gpio)) {
			if (ctrl_pdata->avdd_en_gpio_invert)
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
//Guoqiang.Jiang@PSW.MM.Display, 2018/10/30 modify for high brightness mode
							...
							u32 bl_level)
{
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
	//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2017/02/14, add for close bl for silence and sau mode
	if (lcd_closebl_flag)
		bl_level = 0;
#endif
	...
	//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2017/04/03, add for i2c backlight control
	if (is_lcd(OPPO16103_JDI_R63452_1080P_CMD_PANEL)) {
		// Ling.Guo@Swdp.MultiMedia.Display, 2017/04/28,modify for high brightness mode
		current_brightness = bl_level;
		if (bl_level > 1 && outdoor_mode == 0)
...
#ifdef CONFIG_OPPO_VENDOR_EDIT
	//Guoqiang.Jiang@PSW.MM.Display.LCD.Stability, 2017/01/24, add for panel debug
	mutex_lock(&lcd_mutex);
	flag_lcd_off = true;
	mutex_unlock(&lcd_mutex);
